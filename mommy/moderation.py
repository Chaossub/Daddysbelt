...
class VCAutomodCog(commands.Cog):
    ...
    async def manual_unban(self, guild: discord.Guild, user_id: int) -> tuple[bool, str]:
        """Manually unban a user and hard-cancel every active Mommy temp-ban timer."""
        user_id = int(user_id)
        try:
        ...
        # Hard-cancel all active temp-ban state. We intentionally preserve old
        # history documents so escalation still remembers how many temp bans
        # this user has received, but no remaining timer can act on them.
        try:
            ...
            log.exception("Unban succeeded but temp-ban history could not be hard-cancelled for %s", user_id)

        # Also close matching punishment records so the dashboard/database has no
        # stale future unban timestamp left behind anywhere.
        try:
            _, punishments, _ = await self._collections()
            result = await asyncio.to_thread(
                punishments.update_many,
                {"guild_id": guild.id, "user_id": user_id, "action": "temp_ban", "succeeded": True, "temp_ban_ended": {"$ne": True}},
                {"$set": {
                    "temp_ban_ended": True,
            ...
            log.exception("Unban succeeded but punishment records could not be closed for %s", user_id)

        log.info(
            "Manual temp ban cancelled for user=%s guild=%s history_closed=%s punishment_closed=%s",
            user_id, guild.id, history_closed, punishment_closed,
        )
        return True, (
            "Unbanned successfully and hard-cancelled Mommy's active temp-ban timer. "
            f"Closed {history_closed} active timer record(s). Previous temp-ban count is still kept for escalation."
        )
