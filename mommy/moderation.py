...
class VCAutomodCog(commands.Cog):
    ...
    async def _apply_punishment(self, member: discord.Member, trigger: dict[str, Any], cfg: dict[str, Any], punishments, infractions, score: int, step: dict[str, Any], history_days: int) -> None:
        ...
        async def remove_house_role() -> None:
        ...
        # Soundboard/ban clip gets a chance to play before a kick/ban removes the member.
        if step.get("play_sound", True) and cfg.get("punishment_sound_enabled", True):
        ...
        if action == "warning":
            ...
            await remove_house_role()
        elif action == "timeout":
            await timeout_member()
        elif action == "remove_timeout":
            # These are intentionally independent. Missing/failed House Trained
            # removal never prevents the Discord timeout from being attempted.
            await remove_house_role()
            await timeout_member()
        elif action == "kick":
            try:
        ...
        if action not in {"warning", "kick", "ban"} and member.voice and member.voice.channel:
            try:
