...
VALID_PUNISHMENT_ACTIONS = {"warning", "remove_role", "timeout", "remove_timeout", "kick", "temp_ban", "ban"}
...
class VCAutomodCog(commands.Cog):
    ...
    async def ensure_defaults(self, guild: discord.Guild) -> dict[str, Any]:
        ...
        # Automatic temp-ban escalation: 24h, 3d, 5d, then 7d for every later temp ban.
        temp_ban_durations = cfg.get("temp_ban_durations_minutes")
        if not isinstance(temp_ban_durations, list) or not temp_ban_durations:
            temp_ban_durations = [1440, 4320, 7200, 10080]
            await self.bot.database.set_config_path(guild.id, "vc_automod.temp_ban_durations_minutes", temp_ban_durations)
            cfg["temp_ban_durations_minutes"] = temp_ban_durations
        if changed:
            await self.bot.database.set_config_path(guild.id, "vc_automod.triggers", triggers)
            cfg["triggers"] = triggers
        ...
        # Migrate the old 4-point / 24-hour remove+timeout default into the new
        # automatic temporary-ban level. Custom ladders are otherwise left alone.
        ladder_changed = False
        for row in ladder:
            try:
                if (int(row.get("threshold", 0)) == 4
                        and str(row.get("action") or "") == "remove_timeout"
                        and int(row.get("duration_minutes", 0) or 0) == 1440):
                    row["action"] = "temp_ban"
                    row["reset_score"] = True
                    row["play_sound"] = False
                    ladder_changed = True
    ...
    async def _apply_punishment(self, member: discord.Member, trigger: dict[str, Any], cfg: dict[str, Any], punishments, infractions, score: int, step: dict[str, Any], history_days: int) -> None:
        ...
        if action == "temp_ban":
            duration, prior_temp_bans = await self._temp_ban_duration(member.guild.id, member.id, cfg)
            temp_ban_number = prior_temp_bans + 1
        ...
        if action == "warning":
            ...
            try:
                await member.ban(reason=f"{reason} • temporary ban {temp_ban_number} for {duration} minutes", delete_message_seconds=0)
                successes.append(f"temporarily banned {duration}m")
                history = await self._tempban_history_collection()
                await asyncio.to_thread(history.insert_one, {
                    "guild_id": member.guild.id,
                    "user_id": member.id,
                    "display_name": member.display_name,
                    "created_at": now,
                    "duration_minutes": duration,
                    "ban_number": temp_ban_number,
                    "unban_at": now + timedelta(minutes=duration),
                    "unbanned": False,
                    "succeeded": True,
                })
            except (discord.Forbidden, discord.HTTPException) as exc:
                failures.append(f"could not temp ban: {type(exc).__name__}")
                log.exception("Could not temp ban %s", member.id)
        elif action == "ban":
            try:
        ...
        should_disconnect = action not in {"warning", "kick", "temp_ban", "ban"}
        ...
        if action == "warning":
            ...
            punishment_succeeded = "kicked" in successes
        elif action == "temp_ban":
            punishment_succeeded = any(x.startswith("temporarily banned") for x in successes)
        elif action == "ban":
            punishment_succeeded = "banned" in successes
        else:
        ...
        doc = {
            ...
            "unban_at": (now + timedelta(minutes=duration)) if action == "temp_ban" and "temporarily banned" in " ".join(successes) else None,
        ...
        # reset-score action starts a completely fresh ladder cycle; temp-ban
        # escalation history is stored separately and is intentionally preserved.
    ...
    async def manual_unban(self, guild: discord.Guild, user_id: int) -> tuple[bool, str]:
        """Manually unban a user and close any active Mommy temp-ban records."""
        user_id = int(user_id)
        try:
            await guild.unban(discord.Object(id=user_id), reason="Manual Mommy.exe unban")
        except discord.NotFound:
            return False, "That user is not currently banned."
        except discord.Forbidden:
            return False, "Discord refused the unban. Give Mommy **Ban Members** permission and make sure her role is high enough."
        except discord.HTTPException as exc:
            log.exception("Could not manually unban %s in guild %s", user_id, guild.id)
            return False, f"Discord could not process the unban ({type(exc).__name__})."

        # Preserve ban-count/escalation history, but mark any still-active temp ban
        # as manually ended so the background worker does not try to unban again.
        try:
            history = await self._tempban_history_collection()
            now = utcnow()
            await asyncio.to_thread(
                history.update_many,
                {"guild_id": guild.id, "user_id": user_id, "succeeded": True, "unbanned": False},
                {"$set": {"unbanned": True, "unbanned_at": now, "manual_unban": True}},
            )
        except Exception:
            log.exception("Unban succeeded but temp-ban history could not be updated for %s", user_id)

        return True, "Unbanned successfully. Their previous temp-ban count is still kept for escalation."
    ...
    async def restore_due_roles(self) -> None:
        ...
        # Temporary bans use persistent history so escalation survives score
        # resets. Unban automatically when their timer expires.
        temp_history = await self._tempban_history_collection()
        def _due_unbans():
            return list(temp_history.find({"succeeded": True, "unbanned": False, "unban_at": {"$lte": now}}).limit(100))
        for item in await asyncio.to_thread(_due_unbans):
            guild = self.bot.get_guild(int(item["guild_id"]))
            if guild is None:
                continue
            try:
                await guild.unban(discord.Object(id=int(item["user_id"])), reason="Mommy.exe temporary ban expired")
                await asyncio.to_thread(temp_history.update_one, {"_id": item["_id"]}, {"$set": {"unbanned": True, "unbanned_at": now}})
            except discord.NotFound:
                # Already manually unbanned; consider the temp ban complete.
                await asyncio.to_thread(temp_history.update_one, {"_id": item["_id"]}, {"$set": {"unbanned": True, "unbanned_at": now, "manual_or_missing": True}})
            except (discord.Forbidden, discord.HTTPException):
                log.exception("Could not automatically unban %s in guild %s", item.get("user_id"), item.get("guild_id"))
