...
class AutomodView(discord.ui.View):
    ...
    @discord.ui.button(label="Temp Ban Status", emoji="🧾", style=discord.ButtonStyle.secondary, row=3)
    async def tempban_status(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        if not await _authorized(interaction, self.bot, minimum="admin"): return
        ...
        await interaction.response.defer(ephemeral=True, thinking=True)
        try:
            active = await cog.active_temp_bans(interaction.guild.id, limit=25)
        except Exception:
            log.exception("Could not read active temp-ban status")
            await interaction.followup.send("I couldn't read Mommy's temp-ban records right now.", ephemeral=True); return
        if not active:
            await interaction.followup.send(
                "✅ **Active Mommy temp bans: NONE**\nThere is no live Mommy temp-ban timer currently blocking anyone in this server.",
                ephemeral=True,
            )
            return
        lines = []
        for item in active:
            ...
            until = _fmt_pacific(item.get("unban_at"))
            number = item.get("ban_number") or item.get("temp_ban_number") or "?"
            lines.append(f"• <@{uid}> (`{uid}`) — ban #{number} — until **{until}**")
        await interaction.followup.send(
            "**Active Mommy temp bans**\n" + "\n".join(lines[:25]),
            ephemeral=True,
        )
    ...
    @discord.ui.button(label="Force Clear Temp Ban", emoji="🧹", style=discord.ButtonStyle.danger, row=4)
    async def force_clear_tempban(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        if not await _authorized(interaction, self.bot, minimum="admin"): return
        await interaction.response.send_modal(ForceClearTempBanModal(self.bot))
