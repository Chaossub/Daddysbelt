...
class MommyVoiceSettingsView(discord.ui.View):
    def __init__(self, bot, *, current_voice: str) -> None:
        ...
        self.elevenlabs = bool(os.getenv("ELEVENLABS_API_KEY", "").strip() and os.getenv("ELEVENLABS_VOICE_ID", "").strip())
        if self.elevenlabs:
            # ElevenLabs is configured from Render, so there is no OpenAI voice to save here.
            # Keep this panel alive so the admin can repeatedly test the configured voice.
            for child in list(self.children):
                if isinstance(child, discord.ui.Button) and child.label == "Preview Selected":
                    child.label = "Test ElevenLabs Voice"
    ...
    def embed_for(self, selected: str | None = None) -> discord.Embed:
        ...
        if self.elevenlabs:
            model = os.getenv("ELEVENLABS_MODEL_ID", "eleven_multilingual_v2").strip() or "eleven_multilingual_v2"
            return discord.Embed(
                title="Mommy.exe • Voice Settings",
                description=(
                    "**Active provider:** `ElevenLabs`\n"
                    f"**Model:** `{model}`\n"
                    "**Voice:** your `ELEVENLABS_VOICE_ID` from Render\n\n"
                    "Press **Test ElevenLabs Voice** as many times as you want. "
                    "This panel stays usable, so you do not need to type `!mommy` between tests.\n\n"
                    "To switch to another ElevenLabs voice later, replace `ELEVENLABS_VOICE_ID` in Render and redeploy."
                ),
            )
        return discord.Embed(
            title="Mommy.exe • Voice Settings",
...
async def show_ai_page(interaction: discord.Interaction, bot) -> None:
    ...
    elevenlabs = bool(os.getenv("ELEVENLABS_API_KEY", "").strip() and os.getenv("ELEVENLABS_VOICE_ID", "").strip())
    provider_text = "ElevenLabs (Render voice ID)" if elevenlabs else f"OpenAI / {current_voice}"
    embed = discord.Embed(
        title="Mommy.exe • AI",
        description=(
...
async def show_voice_settings_page(interaction: discord.Interaction, bot) -> None:
    ...
    current_voice = str(configured or os.getenv("MOMMY_TTS_VOICE", "shimmer")).strip().lower()
    elevenlabs = bool(os.getenv("ELEVENLABS_API_KEY", "").strip() and os.getenv("ELEVENLABS_VOICE_ID", "").strip())
    if not elevenlabs and current_voice not in MOMMY_TTS_VOICES:
...
class ExceptionRoleSelect(discord.ui.RoleSelect):
    def __init__(self,parent,action): super().__init__(placeholder="Choose a role…",min_values=1,max_values=1); self.parent=parent; self.action=action
    async def callback(self,interaction):
        role=self.values[0]; profile=await self.parent.bot.database.get_guild_profile(interaction.guild.id) or {}; ids=[int(v) for v in (profile.get("vc_automod") or {}).get("exception_role_ids") or []]
        if self.action=="add" and role.id not in ids: ids.append(role.id)
        if self.action=="remove": ids=[v for v in ids if v!=role.id]
        await self.parent.bot.database.set_config_path(interaction.guild.id,"vc_automod.exception_role_ids",ids)
...
class AutomodMemberSelect(discord.ui.UserSelect):
    def __init__(self,parent,action): super().__init__(placeholder="Choose member…",min_values=1,max_values=1); self.parent=parent; self.action=action
    async def callback(self,interaction):
        member=interaction.guild.get_member(int(self.values[0].id)); cog=_vc_automod_cog(self.parent.bot)
        ...
        if self.action=="history":
            ...
            await interaction.response.edit_message(embed=discord.Embed(title=f"VC History • {member.display_name}",description=desc),view=SimpleNavView(self.parent.bot,back="automod")); return
...
class RulesChannelSelect(discord.ui.ChannelSelect):
    ...
    async def callback(self,interaction):
        ch=self.values[0]; await self.parent.bot.database.set_config_path(interaction.guild.id,"verification.rules_channel_id",ch.id); await interaction.response.send_message(f"Rules channel set to {ch.mention}.",ephemeral=True)
