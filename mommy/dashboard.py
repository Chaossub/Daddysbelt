...
class AutomodView(discord.ui.View):
    ...
    @discord.ui.button(label="Unban Member", emoji="🔓", style=discord.ButtonStyle.danger, row=3)
    async def unban_member(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        if not await _authorized(interaction, self.bot, minimum="admin"): return
        await interaction.response.defer(ephemeral=True, thinking=True)
        try:
            entries = [entry async for entry in interaction.guild.bans(limit=25)]
        except discord.Forbidden:
            await interaction.followup.send("I can't read the server ban list. Give Mommy **Ban Members** permission.", ephemeral=True); return
        except discord.HTTPException:
            await interaction.followup.send("Discord wouldn't give me the ban list right now.", ephemeral=True); return
        if not entries:
            await interaction.followup.send("There aren't any banned users to unban.", ephemeral=True); return
        await interaction.followup.send(
            "Choose who you want Mommy to unban.",
            view=UnbanPickView(self.bot, entries),
            ephemeral=True,
        )
...
def _normalize_action(value: str) -> str | None:
    ...
    aliases={"remove":"remove_role","house_trained":"remove_role","role":"remove_role","remove_house_trained":"remove_role","remove_and_timeout":"remove_timeout","role_timeout":"remove_timeout","remove_role_timeout":"remove_timeout","tempban":"temp_ban","temporary_ban":"temp_ban"}
    v=aliases.get(v,v)
...
class PunishmentLevelModal(discord.ui.Modal, title="Punishment Ladder Level"):
    ...
    action=discord.ui.TextInput(label="Action",placeholder="warning / remove_timeout / kick / temp_ban / ban",max_length=30)
    ...
    async def on_submit(self,interaction):
        ...
        if action is None: await interaction.response.send_message("Action must be warning, remove_role, timeout, remove_timeout, kick, temp_ban, or ban.",ephemeral=True); return
...
class TempBanEscalationModal(discord.ui.Modal, title="Temp Ban Escalation"):
    first=discord.ui.TextInput(label="1st temp ban (hours)",default="24",max_length=3)
    second=discord.ui.TextInput(label="2nd temp ban (hours)",default="72",max_length=3)
    third=discord.ui.TextInput(label="3rd temp ban (hours)",default="120",max_length=3)
    fourth=discord.ui.TextInput(label="4th+ temp ban (hours)",default="168",max_length=3)
    def __init__(self,bot,cfg):
        super().__init__(); self.bot=bot
        vals=list(cfg.get("temp_ban_durations_minutes") or [1440,4320,7200,10080])
        while len(vals)<4: vals.append(vals[-1] if vals else 1440)
        self.first.default=str(max(24,int(vals[0])//60)); self.second.default=str(max(24,int(vals[1])//60)); self.third.default=str(max(24,int(vals[2])//60)); self.fourth.default=str(max(24,int(vals[3])//60))
    async def on_submit(self,interaction):
        try: hours=[int(str(self.first)),int(str(self.second)),int(str(self.third)),int(str(self.fourth))]
        except ValueError: await interaction.response.send_message("All four temp-ban durations must be whole numbers of hours.",ephemeral=True); return
        if any(v < 24 or v > 168 for v in hours):
            await interaction.response.send_message("Automatic temp bans must be between 24 and 168 hours (1–7 days).",ephemeral=True); return
        if any(hours[i] > hours[i+1] for i in range(3)):
            await interaction.response.send_message("Each temp ban should be the same length or longer than the previous one.",ephemeral=True); return
        await self.bot.database.set_config_path(interaction.guild.id,"vc_automod.temp_ban_durations_minutes",[v*60 for v in hours])
        await interaction.response.send_message(f"Temp-ban escalation saved: {hours[0]}h → {hours[1]}h → {hours[2]}h → {hours[3]}h max.",ephemeral=True)
...
class PunishmentLadderView(discord.ui.View):
    ...
    @discord.ui.button(label="Temp Ban Escalation",emoji="⏳",style=discord.ButtonStyle.secondary,row=1)
    async def tempban(self,interaction,_):
        if not await _authorized(interaction,self.bot,minimum="admin"): return
        profile=await self.bot.database.get_guild_profile(interaction.guild.id) or {}; cfg=(profile.get("vc_automod") or {})
        await interaction.response.send_modal(TempBanEscalationModal(self.bot,cfg))
    @discord.ui.button(label="Back",emoji="⬅️",style=discord.ButtonStyle.secondary,row=1)
    async def back(self,interaction,_): await navigate(interaction,self.bot,"automod")
    @discord.ui.button(label="Home",emoji="🏠",style=discord.ButtonStyle.success,row=1)
    async def home(self,interaction,_): await navigate(interaction,self.bot,"home")
...
async def show_punishment_ladder_page(interaction: discord.Interaction, bot) -> None:
    ...
    tb=list(cfg.get("temp_ban_durations_minutes") or [1440,4320,7200,10080]); tb_hours=[max(1,int(v)//60) for v in tb[:4]]
    desc += f"\n\n**Temp-ban escalation:** {' → '.join(str(v)+'h' for v in tb_hours)} (4th+ uses the last value). A successful temp ban resets the score when that ladder level has Reset Score = yes.\n\nActions: `warning`, `remove_role`, `timeout`, `remove_timeout`, `kick`, `temp_ban`, `ban`.\n`ban` is permanent; use `temp_ban` for Mommy's automatic 1–7 day bans."
