...
def _fmt_pacific(value) -> str:
    """Render stored UTC datetimes in Pacific local time with PST/PDT label."""
    if not isinstance(value, datetime):
        return "unknown time"
    if value.tzinfo is None:
        value = value.replace(tzinfo=timezone.utc)
    local = value.astimezone(PACIFIC_TZ)
    # Portable 12-hour formatting without relying on %-I on every platform.
    hour = local.strftime("%I").lstrip("0") or "12"
    return f"{local.strftime('%m/%d/%Y')} {hour}:{local.strftime('%M %p')} {local.tzname()}"
...
class MommyDashboardView(discord.ui.View):
    ...
    @discord.ui.button(label="Follow", emoji="👣", style=discord.ButtonStyle.secondary, row=0)
    ...
    @discord.ui.button(label="Auto Mod", emoji="🛡️", style=discord.ButtonStyle.secondary, row=1)
    async def automod(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        ...
        if interaction.guild is None:
            ...
            return
        cog = _vc_automod_cog(self.bot)
        cfg = await cog.ensure_defaults(interaction.guild) if cog else {}
        ...
        desc = (
            ...
            f"**House Trained:** {f'<@&{role_id}>' if role_id else 'not set'}\n"
            f"**Triggers:** {len(triggers)}\n"
            f"**Exception roles:** {len(cfg.get('exception_role_ids') or [])}\n\n"
            "Use **18+ VC Guard** for suspected child/minor audio, exemptions, manual VC disconnects, and appeals."
        )
        await interaction.response.send_message(
            embed=discord.Embed(title="Mommy.exe • Auto Mod", description=desc),
    ...
    @discord.ui.button(label="Games", emoji="🎮", style=discord.ButtonStyle.success, row=2)
    async def games(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        if not await _authorized(interaction, self.bot, minimum="moderator"):
            return
        from mommy.member_games import MemberHubView, member_hub_embed

        if interaction.guild is None or interaction.channel is None:
            await interaction.response.send_message("This can only be used inside a server channel.", ephemeral=True)
            return
        # Staff get the exact same member-side launcher so they can host/play too.
        # The setup menu stays private to the person who opened it; opening the
        # lobby posts the actual game publicly in the channel.
        await interaction.response.send_message(
            embed=member_hub_embed(interaction.guild),
...
class VoiceView(discord.ui.View):
    ...
    @discord.ui.button(label="Follow", emoji="👣", style=discord.ButtonStyle.secondary, row=2)
    async def follow(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        await show_follow_page(interaction, self.bot)
    ...
    @discord.ui.button(label="Connect Followed Now", emoji="🔌", style=discord.ButtonStyle.primary, row=2)
    async def connect_followed(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        if not await _authorized(interaction, self.bot, minimum="admin"):
            return
        if interaction.guild is None:
            await interaction.response.send_message("Server only.", ephemeral=True)
        ...
        if cog is None:
            ...
            return
        await interaction.response.defer(ephemeral=True, thinking=True)
        result = await cog.dashboard_connect_followed(interaction.guild, interaction.user)
...
class FollowView(discord.ui.View):
    ...
    async def add(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        ...
        await interaction.response.edit_message(
            embed=discord.Embed(
                title="Follow Members • Add",
                description="Choose a member, then choose what Mommy should do when she follows them. Clipping stays available for everyone.",
            ),
            view=FollowMemberSelectView(self.bot, action="add"),
        )

    @discord.ui.button(label="Edit Behavior", emoji="⚙️", style=discord.ButtonStyle.primary, row=0)
    async def edit(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        if not await _authorized(interaction, self.bot, minimum="admin"):
            return
        await interaction.response.edit_message(
            embed=discord.Embed(title="Follow Members • Edit", description="Choose a followed member, then set their follow behavior."),
            view=FollowMemberSelectView(self.bot, action="edit"),
        )
    ...
    @discord.ui.button(label="Refresh List", emoji="🔄", style=discord.ButtonStyle.secondary, row=1)
...
class FollowMemberSelect(discord.ui.UserSelect):
    def __init__(self, parent_view, *, action: Literal["add", "edit", "remove"]) -> None:
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        if not targets:
            default = self.parent_view.bot.database._clean_voice_join_behavior(config.get("default_join_behavior"))  # noqa: SLF001
        ...
        if self.action == "remove":
            ...
            await self.parent_view.bot.database.set_voice_recording_follow_targets(interaction.guild.id, targets)
            voice_cog = self.parent_view.bot.get_cog("VoiceRecordingCog")
            if voice_cog is not None and hasattr(voice_cog, "_reconcile_auto_join"):
                await voice_cog._reconcile_auto_join(interaction.guild)  # noqa: SLF001
            await interaction.response.edit_message(
                embed=discord.Embed(title="Follow Members", description=f"I won't automatically follow <@{user_id}> anymore."),
                view=FollowView(self.parent_view.bot),
            )
            return

        existing = next((item for item in targets if int(item.get("user_id", 0)) == user_id), None)
        if self.action == "edit" and existing is None:
            await interaction.response.send_message("That member isn't currently on Mommy's followed list.", ephemeral=True)
            return

        await interaction.response.edit_message(
            embed=discord.Embed(
                title="Follow Members • Behavior",
                description=(
                    f"Choose what Mommy should do when she follows <@{user_id}>.\n\n"
                    "**Automod Only** — listen/transcribe for VC automod; no full-session archive.\n"
                    "**Record Only** — record/archive the session; VC automod is off.\n"
                    "**Automod + Record** — both.\n\n"
                    "✂️ **Clipping is global** and remains available for everyone in the active VC, regardless of this setting."
                ),
            ),
            view=FollowBehaviorView(self.parent_view.bot, user_id=user_id),
        )


class FollowBehaviorSelect(discord.ui.Select):
    def __init__(self, parent_view: "FollowBehaviorView") -> None:
        options = [
            discord.SelectOption(label="Automod Only", value="automod", emoji="🛡️", description="Follow + VC automod, no full recording"),
            discord.SelectOption(label="Record Only", value="record", emoji="🎙️", description="Follow + full recording, no VC automod"),
            discord.SelectOption(label="Automod + Record", value="both", emoji="🛡️", description="Follow + VC automod + full recording"),
        ]
        super().__init__(placeholder="Choose follow behavior…", options=options, min_values=1, max_values=1)
        self.parent_view = parent_view
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        mode = self.values[0]
        behavior = {
            "automod": mode in {"automod", "both"},
            "record": mode in {"record", "both"},
            "ai": True,
            "captions": True,
        }
        profile = await self.parent_view.bot.database.get_guild_profile(interaction.guild.id)
        ...
        if not targets:
            default = self.parent_view.bot.database._clean_voice_join_behavior(config.get("default_join_behavior"))  # noqa: SLF001
            targets = [{"user_id": int(v), "behavior": default} for v in (config.get("follow_user_ids") or [])]
        targets = [item for item in targets if int(item.get("user_id", 0)) != self.parent_view.user_id]
        targets.append({"user_id": self.parent_view.user_id, "behavior": behavior})
        await self.parent_view.bot.database.set_voice_recording_follow_targets(interaction.guild.id, targets)

        member = interaction.guild.get_member(self.parent_view.user_id)
        voice_cog = self.parent_view.bot.get_cog("VoiceRecordingCog")
        immediate = ""
        if member is not None and voice_cog is not None and hasattr(voice_cog, "follow_member_now"):
            try:
                immediate = await voice_cog.follow_member_now(interaction.guild, member)
            except Exception as exc:
                log.exception("Immediate followed-member join failed for %s", self.parent_view.user_id)
                immediate = f"Saved the behavior, but the immediate VC reconcile failed: {type(exc).__name__}."

        label = {"automod": "Automod Only", "record": "Record Only", "both": "Automod + Record"}[mode]
        description = f"<@{self.parent_view.user_id}> is now **{label}**.\n✂️ Clipping remains available for everyone."
        if immediate:
            description += f"\n\n{immediate}"
        await interaction.response.edit_message(
            embed=discord.Embed(title="Follow Behavior Saved", description=description),
            view=FollowView(self.parent_view.bot),
        )


class FollowBehaviorView(discord.ui.View):
    def __init__(self, bot, *, user_id: int) -> None:
        super().__init__(timeout=300)
        self.bot = bot
        self.user_id = int(user_id)
        self.add_item(FollowBehaviorSelect(self))
...
class FollowMemberSelectView(discord.ui.View):
    def __init__(self, bot, *, action: Literal["add", "edit", "remove"]) -> None:
        super().__init__(timeout=300)
        self.bot = bot
...
async def show_clips_page(interaction: discord.Interaction, bot) -> None:
    ...
    embed = discord.Embed(
        ...
        description=(
            "Mommy keeps the active session audio aligned so staff can save the **previous 3 minutes** from anyone in the active VC.\n"
            "Clipping is global and is not controlled by followed-member behavior.\n\n"
...
async def show_follow_page(interaction: discord.Interaction, bot) -> None:
    profile = await bot.database.get_guild_profile(interaction.guild.id) if interaction.guild else None
    cfg = ((profile or {}).get("voice_recording", {}) or {})
    targets = list(cfg.get("follow_targets") or [])
    if not targets:
        default = bot.database._clean_voice_join_behavior(cfg.get("default_join_behavior"))  # noqa: SLF001
        targets = [{"user_id": int(v), "behavior": default} for v in (cfg.get("follow_user_ids") or [])]
    lines = []
    if interaction.guild:
        for item in targets:
            ...
            automod = bool(behavior.get("automod", True))
            record = bool(behavior.get("record", False))
            if automod and record:
                mode = "🛡️🎙️ Automod + Record"
            elif record:
                mode = "🎙️ Record Only"
            else:
                mode = "🛡️ Automod Only"
            lines.append(f"• {member.mention if member else f'<@{user_id}>'} — **{mode}**")
    description = "\n".join(lines) if lines else "Nobody is on the followed-member list yet."
    description += "\n\n✂️ **Clipping is global:** staff can clip anyone in an active Mommy VC session; it is not a followed-member behavior."
...
class StatusView(discord.ui.View):
    ...
    @discord.ui.button(label="Connect Followed Now", emoji="🔌", style=discord.ButtonStyle.success)
    async def connect_followed(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        if not await _authorized(interaction, self.bot, minimum="admin"):
            return
        if interaction.guild is None:
            await interaction.response.send_message("Server only.", ephemeral=True)
...
async def show_status_page(interaction: discord.Interaction, bot) -> None:
    ...
    cog = _voice_cog(bot)
    profile = await bot.database.get_guild_profile(guild.id) if guild else None
    cfg = (profile or {}).get("voice_recording", {})
    live = cog.live_status(guild) if cog and guild and hasattr(cog, "live_status") else {}
    session = live.get("session")
    output_id = cfg.get("status_log_channel_id") or cfg.get("output_channel_id")

    if session:
        mode = []
        if session.record_enabled:
            mode.append("🔴 Recording")
        else:
            mode.append("⚪ Recording off")
        mode.append("🛡️ Automod on" if session.automod_enabled else "🛡️ Automod off")
        mode.append("💬 Captions on" if session.captions_enabled else "💬 Captions off")
        mode.append("🧠 AI on" if session.ai_enabled else "🧠 AI off")
        active = (
            f"<#{session.voice_channel_id}>\n"
            f"**Mode:** {' • '.join(mode)}\n"
            f"**Duration:** `{_clock(session.duration) if '_clock' in globals() else int(session.duration)}`\n"
            f"**Speaker tracks:** `{len(session.tracks)}` • **Speech chunks:** `{len(session.segments)}`\n"
            f"**Session type:** `{session.join_reason}`"
            + (f" • target <@{session.join_target_id}>" if session.join_target_id else "")
        )
    else:
    ...
    voice_health = (
        "🟢 connected + receiving audio" if connected and listening
        else "🟡 connected, but not receiving audio" if connected
        else "⚪ not connected"
    )
    transcript = "🟢 ready" if live.get("transcription") else "🔴 unavailable"
    last_error = live.get("last_error") or "none"
    last_event = live.get("last_event")
    if last_event:
        event_text, event_time = last_event
        last_event_text = f"{event_text} • <t:{int(event_time.timestamp())}:R>"
    else:
        last_event_text = "none yet"

    embed = discord.Embed(title="Mommy.exe • Live Status")
    embed.add_field(
        name="System",
        value=(
            f"**Discord:** {'🟢 ready' if bot.is_ready() else '🟡 starting/reconnecting'}\n"
            f"**MongoDB:** {'🟢 connected' if bot.database.connected else '🔴 disconnected'}\n"
            f"**Transcription:** {transcript}\n"
            f"**Voice receive:** {voice_health}"
        ),
        inline=False,
    )
    embed.add_field(name="Current VC Session", value=active, inline=False)
    embed.add_field(
        name="Controls",
        value=(
            f"**Hushed:** {'yes' if live.get('hushed') else 'no'}\n"
            f"**Followed members:** `{len(cfg.get('follow_targets') or cfg.get('follow_user_ids') or [])}`\n"
            f"**Status/log channel:** {f'<#{output_id}>' if output_id else 'not configured'}"
        ),
        inline=False,
    )
    embed.add_field(name="Last Status Event", value=last_event_text[:1024], inline=False)
    if last_error != "none":
        embed.add_field(name="Last Voice Error", value=f"`{last_error}`"[:1024], inline=False)
    embed.set_footer(text="Refresh this page for the current live state.")
    await interaction.response.edit_message(embed=embed, view=StatusView(bot))
...
class AutomodView(discord.ui.View):
    ...
    @discord.ui.button(label="18+ VC Guard", emoji="🔞", style=discord.ButtonStyle.danger, row=4)
    async def child_vc(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        if (
            ...
            await interaction.response.send_message(
                "Only the server owner or an Administrator can use the 18+ VC controls.",
                ephemeral=True,
            )
            return
        from mommy.child_vc import ChildVCAdminView, child_vc_admin_embed
        await interaction.response.edit_message(
            embed=await child_vc_admin_embed(self.bot, interaction.guild),
            view=ChildVCAdminView(self.bot),
        )
...
class ForceClearTempBanModal(discord.ui.Modal, title="Force Clear Mommy Temp Ban"):
    user_id = discord.ui.TextInput(
        label="Discord user ID",
        placeholder="Paste the numeric user ID",
        max_length=24,
    )
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        try:
            ...
            await interaction.response.send_message("That doesn't look like a Discord user ID.", ephemeral=True)
            return
        cog = _vc_automod_cog(self.bot)
        if cog is None:
            await interaction.response.send_message("VC automod is unavailable.", ephemeral=True)
            return
        await interaction.response.defer(ephemeral=True, thinking=True)
        try:
            history_closed, punishment_closed = await cog.force_clear_tempban_state(interaction.guild.id, uid)
        except Exception:
            log.exception("Could not force-clear temp-ban state for %s", uid)
            await interaction.followup.send("I couldn't clear Mommy's stored temp-ban state.", ephemeral=True)
            return
        await interaction.followup.send(
            f"✅ Force-cleared Mommy temp-ban state for `{uid}`. "
            f"Closed **{history_closed}** active timer record(s) and **{punishment_closed}** punishment record(s). "
            "This does not call Discord's unban endpoint; use it when the user is already absent from the server ban list.",
            ephemeral=True,
        )
...
class VCTriggerModal(discord.ui.Modal, title="Add VC Automod Trigger"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        trigger={"id":uuid.uuid4().hex[:12],"label":str(self.label).strip(),"phrase":str(self.phrase).strip(),"enabled":True,"threshold":threshold,"window_minutes":window,"points":points,"history_days":history_days,"quote_lenient":True,"spoken_warning":False,"warning_style":"random","warning_text":"","scope_mode":"everyone","scope_user_ids":[]}
...
class VCTriggerView(discord.ui.View):
    ...
    @discord.ui.button(label="Warning Settings", emoji="📣", style=discord.ButtonStyle.secondary, row=1)
    async def warning(self, interaction, _):
        if not await _authorized(interaction,self.bot,minimum="admin"): return
        cog=_vc_automod_cog(self.bot); cfg=await cog.ensure_defaults(interaction.guild) if cog else ((await self.bot.database.get_guild_profile(interaction.guild.id) or {}).get("vc_automod") or {})
        await interaction.response.edit_message(
            embed=discord.Embed(
                title="VC Automod • Spoken Warnings",
                description="Choose a trigger to set spoken warning ON/OFF and choose Random Pool or a Custom Line. Warnings can speak even in crowded channels.",
            ),
            view=VCTriggerWarningPickView(self.bot,cfg.get("triggers") or []),
        )

    @discord.ui.button(label="Warning Lines", emoji="🗣️", style=discord.ButtonStyle.secondary, row=1)
    async def warning_lines(self, interaction, _):
        if not await _authorized(interaction,self.bot,minimum="admin"): return
        await show_warning_lines_page(interaction,self.bot)

    @discord.ui.button(label="Member Scope", emoji="🎯", style=discord.ButtonStyle.secondary, row=2)
    async def member_scope(self, interaction, _):
        if not await _authorized(interaction,self.bot,minimum="admin"): return
        cog=_vc_automod_cog(self.bot); cfg=await cog.ensure_defaults(interaction.guild) if cog else ((await self.bot.database.get_guild_profile(interaction.guild.id) or {}).get("vc_automod") or {})
        await interaction.response.edit_message(
            embed=discord.Embed(
                title="VC Automod • Trigger Member Scope",
                description="Choose a trigger, then decide whether it applies to everyone, only selected members, or everyone except selected members.",
            ),
            view=VCTriggerScopePickView(self.bot,cfg.get("triggers") or []),
        )
    ...
    async def remove(self, interaction, _):
        ...
        await interaction.response.edit_message(embed=discord.Embed(title="Remove VC Trigger",description="Choose a trigger to remove."),view=VCTriggerRemoveView(self.bot, triggers))
    @discord.ui.button(label="Back", emoji="⬅️", style=discord.ButtonStyle.secondary)
...
async def _save_trigger_scope(bot, guild_id: int, trigger_id: str, *, mode: str | None = None, user_ids: list[int] | None = None) -> dict:
    profile=await bot.database.get_guild_profile(guild_id) or {}
    triggers=list((profile.get("vc_automod") or {}).get("triggers") or [])
    updated={}
    for t in triggers:
        if str(t.get("id"))==str(trigger_id):
            ...
            if user_ids is not None:
                t["scope_user_ids"]=list(dict.fromkeys(int(v) for v in user_ids))
            updated=t
            break
    await bot.database.set_config_path(guild_id,"vc_automod.triggers",triggers)
    return updated


def _trigger_scope_text(trigger: dict) -> str:
    mode=str(trigger.get("scope_mode") or "everyone")
    ids=[]
    for value in trigger.get("scope_user_ids") or []:
        try: ids.append(int(value))
        except (TypeError,ValueError): pass
    if mode=="only_selected":
        label="Only selected members"
    elif mode=="except_selected":
        label="Everyone except selected members"
    else:
        label="Everyone"
    members=" ".join(f"<@{uid}>" for uid in ids[:20]) or "None selected"
    if len(ids)>20: members += f" +{len(ids)-20} more"
    return f"**Scope:** {label}\n**Selected members:** {members}"


class VCTriggerScopeSelect(discord.ui.Select):
    def __init__(self, bot, triggers):
        self.bot=bot
        self.trigger_map={str(t.get("id")):t for t in triggers}
        options=[]
        for t in triggers[:25]:
            mode=str(t.get("scope_mode") or "everyone")
            label={"everyone":"Everyone","only_selected":"Only selected","except_selected":"Except selected"}.get(mode,"Everyone")
            options.append(discord.SelectOption(label=str(t.get("label") or "Trigger")[:100],value=str(t.get("id")),description=f"Scope: {label}"))
        super().__init__(placeholder="Choose trigger member scope…",options=options or [discord.SelectOption(label="No triggers",value="none")])
    async def callback(self, interaction):
        if not await _authorized(interaction,self.bot,minimum="admin"): return
        value=self.values[0]
        if value=="none":
            await interaction.response.send_message("No triggers are configured.",ephemeral=True); return
        trigger=self.trigger_map[value]
        await interaction.response.edit_message(
            embed=discord.Embed(title=f"Member Scope • {trigger.get('label','Trigger')}",description=_trigger_scope_text(trigger)),
            view=VCTriggerScopeConfigView(self.bot,trigger),
        )


class VCTriggerScopePickView(discord.ui.View):
    def __init__(self,bot,triggers):
        super().__init__(timeout=300); self.bot=bot; self.add_item(VCTriggerScopeSelect(bot,triggers))
    @discord.ui.button(label="Back",emoji="⬅️",style=discord.ButtonStyle.secondary,row=1)
...
class TriggerScopeUserSelect(discord.ui.UserSelect):
    def __init__(self, owner_view):
        super().__init__(placeholder="Replace selected members…",min_values=1,max_values=25,row=2)
        self.owner_view=owner_view
    async def callback(self,interaction):
        if not await _authorized(interaction,self.owner_view.bot,minimum="admin"): return
        ids=[int(v.id) for v in self.values]
        updated=await _save_trigger_scope(self.owner_view.bot,interaction.guild.id,str(self.owner_view.trigger.get("id")),user_ids=ids)
        self.owner_view.trigger=updated or self.owner_view.trigger
        await interaction.response.edit_message(
            embed=discord.Embed(title=f"Member Scope • {self.owner_view.trigger.get('label','Trigger')}",description=_trigger_scope_text(self.owner_view.trigger)),
            view=VCTriggerScopeConfigView(self.owner_view.bot,self.owner_view.trigger),
        )


class VCTriggerScopeConfigView(discord.ui.View):
    def __init__(self,bot,trigger):
        super().__init__(timeout=300); self.bot=bot; self.trigger=dict(trigger); self.add_item(TriggerScopeUserSelect(self))

    async def _set_mode(self,interaction,mode):
        if not await _authorized(interaction,self.bot,minimum="admin"): return
        updated=await _save_trigger_scope(self.bot,interaction.guild.id,str(self.trigger.get("id")),mode=mode)
        self.trigger=updated or self.trigger
        await interaction.response.edit_message(
            embed=discord.Embed(title=f"Member Scope • {self.trigger.get('label','Trigger')}",description=_trigger_scope_text(self.trigger)),
            view=VCTriggerScopeConfigView(self.bot,self.trigger),
        )

    @discord.ui.button(label="Everyone",emoji="🌐",style=discord.ButtonStyle.primary,row=0)
    async def everyone(self,interaction,_): await self._set_mode(interaction,"everyone")
    @discord.ui.button(label="Only Selected",emoji="🎯",style=discord.ButtonStyle.primary,row=0)
    async def only(self,interaction,_): await self._set_mode(interaction,"only_selected")
    @discord.ui.button(label="Except Selected",emoji="🚫",style=discord.ButtonStyle.primary,row=0)
    async def except_selected(self,interaction,_): await self._set_mode(interaction,"except_selected")

    @discord.ui.button(label="Clear Selected",emoji="🧹",style=discord.ButtonStyle.secondary,row=1)
    async def clear(self,interaction,_):
        if not await _authorized(interaction,self.bot,minimum="admin"): return
        updated=await _save_trigger_scope(self.bot,interaction.guild.id,str(self.trigger.get("id")),user_ids=[])
        self.trigger=updated or self.trigger
        await interaction.response.edit_message(
            embed=discord.Embed(title=f"Member Scope • {self.trigger.get('label','Trigger')}",description=_trigger_scope_text(self.trigger)),
            view=VCTriggerScopeConfigView(self.bot,self.trigger),
    ...
    async def back(self,interaction,_):
        cog=_vc_automod_cog(self.bot); cfg=await cog.ensure_defaults(interaction.guild) if cog else ((await self.bot.database.get_guild_profile(interaction.guild.id) or {}).get("vc_automod") or {})
        await interaction.response.edit_message(embed=discord.Embed(title="VC Automod • Trigger Member Scope",description="Choose another trigger to edit."),view=VCTriggerScopePickView(self.bot,cfg.get("triggers") or []))


class VCTriggerWarningModal(discord.ui.Modal, title="Spoken Warning Settings"):
    enabled = discord.ui.TextInput(label="Spoken warning? (yes/no)", max_length=5)
    style = discord.ui.TextInput(label="Style (random/custom)", max_length=10)
    custom = discord.ui.TextInput(label="Custom warning line (optional)", required=False, max_length=180)

    def __init__(self, bot, trigger):
        super().__init__(); self.bot=bot; self.trigger=trigger
        self.enabled.default="yes" if trigger.get("spoken_warning",False) else "no"
        self.style.default=str(trigger.get("warning_style") or "random")
        self.custom.default=str(trigger.get("warning_text") or "")

    async def on_submit(self, interaction):
        enabled=_parse_yes_no(str(self.enabled))
        style=str(self.style).strip().lower()
        custom=str(self.custom).strip()[:180]
        if enabled is None:
            await interaction.response.send_message("Spoken warning must be yes or no.",ephemeral=True); return
        if style not in {"random","custom"}:
            await interaction.response.send_message("Style must be random or custom.",ephemeral=True); return
        if style=="custom" and not custom:
            await interaction.response.send_message("Add a custom warning line, or use random style.",ephemeral=True); return
        profile=await self.bot.database.get_guild_profile(interaction.guild.id) or {}
        triggers=list((profile.get("vc_automod") or {}).get("triggers") or [])
        label="Trigger"
        for t in triggers:
            if str(t.get("id"))==str(self.trigger.get("id")):
                t["spoken_warning"]=enabled
                t["warning_style"]=style
                t["warning_text"]=custom
                label=str(t.get("label") or "Trigger")
                break
        await self.bot.database.set_config_path(interaction.guild.id,"vc_automod.triggers",triggers)
        await interaction.response.send_message(
            f"Updated spoken warning for **{label}**: **{'ON' if enabled else 'OFF'}**, style **{style}**.",
            ephemeral=True,
        )


class VCTriggerWarningSelect(discord.ui.Select):
    def __init__(self, bot, triggers):
        self.bot=bot
        self.trigger_map={str(t.get("id")):t for t in triggers}
        options=[]
        for t in triggers[:25]:
            enabled=bool(t.get("spoken_warning",False))
            style=str(t.get("warning_style") or "random")
            options.append(discord.SelectOption(
                label=("🔊 " if enabled else "🔇 ")+str(t.get("label") or "Trigger")[:90],
                value=str(t.get("id")),
                description=f"{'ON' if enabled else 'OFF'} • {style} warning",
            ))
        super().__init__(placeholder="Choose trigger warning settings…",options=options or [discord.SelectOption(label="No triggers",value="none")])

    async def callback(self, interaction):
        value=self.values[0]
        if value=="none":
            await interaction.response.send_message("No triggers are configured.",ephemeral=True); return
        await interaction.response.send_modal(VCTriggerWarningModal(self.bot,self.trigger_map[value]))

class VCTriggerWarningPickView(discord.ui.View):
    def __init__(self,bot,triggers):
        super().__init__(timeout=300); self.bot=bot; self.add_item(VCTriggerWarningSelect(bot,triggers))
    @discord.ui.button(label="Back",emoji="⬅️",style=discord.ButtonStyle.secondary,row=1)
    async def back(self,interaction,_): await show_vc_triggers_page(interaction,self.bot)
    @discord.ui.button(label="Home",emoji="🏠",style=discord.ButtonStyle.success,row=1)
    async def home(self,interaction,_): await navigate(interaction,self.bot,"home")
...
class WarningLineAddModal(discord.ui.Modal, title="Add Warning Line"):
    line=discord.ui.TextInput(label="Warning line",placeholder="Example: Don't test me.",max_length=180)
    def __init__(self,bot): super().__init__(); self.bot=bot
    async def on_submit(self,interaction):
        line=str(self.line).strip()[:180]
        if not line:
            await interaction.response.send_message("The warning line cannot be empty.",ephemeral=True); return
        profile=await self.bot.database.get_guild_profile(interaction.guild.id) or {}
        lines=[str(x).strip() for x in ((profile.get("vc_automod") or {}).get("warning_lines") or []) if str(x).strip()]
        if line not in lines: lines.append(line)
        await self.bot.database.set_config_path(interaction.guild.id,"vc_automod.warning_lines",lines)
        await interaction.response.send_message(f"Added warning line: **{line}**",ephemeral=True)

class WarningLineRemoveSelect(discord.ui.Select):
    def __init__(self,bot,lines):
        self.bot=bot; self.lines=list(lines)
        opts=[discord.SelectOption(label=str(line)[:100],value=str(i)) for i,line in enumerate(self.lines[:25])]
        super().__init__(placeholder="Choose a warning line to remove…",options=opts or [discord.SelectOption(label="No warning lines",value="none")])
    async def callback(self,interaction):
        if self.values[0]=="none":
            await interaction.response.send_message("There are no warning lines to remove.",ephemeral=True); return
        ...
        if 0 <= idx < len(lines):
            ...
            await self.bot.database.set_config_path(interaction.guild.id,"vc_automod.warning_lines",lines)
            await interaction.response.send_message(f"Removed warning line: **{removed}**",ephemeral=True)
        else:
            await interaction.response.send_message("That warning line no longer exists.",ephemeral=True)

class WarningLineRemoveView(discord.ui.View):
    def __init__(self,bot,lines): super().__init__(timeout=300); self.bot=bot; self.add_item(WarningLineRemoveSelect(bot,lines))
    @discord.ui.button(label="Back",emoji="⬅️",style=discord.ButtonStyle.secondary,row=1)
    async def back(self,interaction,_): await show_warning_lines_page(interaction,self.bot)

class WarningLinesView(discord.ui.View):
    def __init__(self,bot,lines): super().__init__(timeout=600); self.bot=bot; self.lines=list(lines)
    @discord.ui.button(label="Add Line",emoji="➕",style=discord.ButtonStyle.success)
    async def add(self,interaction,_):
        if not await _authorized(interaction,self.bot,minimum="admin"): return
        await interaction.response.send_modal(WarningLineAddModal(self.bot))
    @discord.ui.button(label="Remove Line",emoji="➖",style=discord.ButtonStyle.danger)
    async def remove(self,interaction,_):
        if not await _authorized(interaction,self.bot,minimum="admin"): return
        await interaction.response.edit_message(embed=discord.Embed(title="Remove Warning Line"),view=WarningLineRemoveView(self.bot,self.lines))
    @discord.ui.button(label="Back",emoji="⬅️",style=discord.ButtonStyle.secondary,row=1)
    async def back(self,interaction,_): await show_vc_triggers_page(interaction,self.bot)
    @discord.ui.button(label="Home",emoji="🏠",style=discord.ButtonStyle.success,row=1)
    async def home(self,interaction,_): await navigate(interaction,self.bot,"home")

async def show_warning_lines_page(interaction,bot):
    profile=await bot.database.get_guild_profile(interaction.guild.id) or {}
    lines=[str(x).strip() for x in ((profile.get("vc_automod") or {}).get("warning_lines") or []) if str(x).strip()]
    if not lines:
        lines=["Watch it. That's your warning.","Careful. You're pushing it.","Knock it off.","Don't test me.","Behave."]
        await bot.database.set_config_path(interaction.guild.id,"vc_automod.warning_lines",lines)
    desc="\n".join(f"• {x}" for x in lines[:20])
    if len(lines)>20: desc+=f"\n…and {len(lines)-20} more."
    await interaction.response.edit_message(embed=discord.Embed(title="VC Automod • Warning Lines",description=desc or "No warning lines configured."),view=WarningLinesView(bot,lines))
...
class PunishmentLevelModal(discord.ui.Modal, title="Punishment Ladder Level"):
    ...
    action=discord.ui.TextInput(label="Action",placeholder="warning / remove_role / timeout / remove_timeout",max_length=30)
    duration=discord.ui.TextInput(label="Duration in minutes (0 for warning)",max_length=7)
    ...
    async def on_submit(self,interaction):
        ...
        if action is None: await interaction.response.send_message("Action must be warning, remove_role, timeout, or remove_timeout.",ephemeral=True); return
...
class UnbanSelect(discord.ui.Select):
    def __init__(self, bot, entries):
        ...
        for entry in entries[:25]:
            user = entry.user
            label = str(user)[:100]
            desc = f"User ID: {user.id}"
            options.append(discord.SelectOption(label=label, value=str(user.id), description=desc[:100]))
        super().__init__(placeholder="Choose a banned member…", min_values=1, max_values=1, options=options)

    async def callback(self, interaction: discord.Interaction):
        if not await _authorized(interaction, self.bot, minimum="admin"): return
        cog = _vc_automod_cog(self.bot)
        if cog is None:
            await interaction.response.send_message("VC automod is unavailable.", ephemeral=True); return
        user_id = int(self.values[0])
        ok, msg = await cog.manual_unban(interaction.guild, user_id)
        await interaction.response.send_message(("✅ " if ok else "⚠️ ") + msg, ephemeral=True)


class UnbanPickView(discord.ui.View):
    def __init__(self, bot, entries):
        super().__init__(timeout=300)
        self.bot = bot
        self.add_item(UnbanSelect(bot, entries))
...
class AutomodMemberSelect(discord.ui.UserSelect):
    ...
    async def callback(self,interaction):
        ...
        if self.action=="history":
            ...
            detections="\n".join(f"• {r.get('trigger_label')} • {'possible quote' if r.get('possible_quote') else 'credible'} • {_fmt_pacific(r.get('created_at'))}" for r in rows[:8]) or "No stored detections."
            points="\n".join(f"• +{r.get('points',1)} {r.get('trigger_label')} • {_fmt_pacific(r.get('created_at'))}" for r in infractions[:6]) or "No active/stored infractions."
            desc=f"**Active score: {score}**\n\n**Detections**\n{detections}\n\n**Point infractions**\n{points}"
            await interaction.response.edit_message(embed=discord.Embed(title=f"VC History • {member.display_name}",description=(desc + '\n\n*Times shown in Pacific Time.*')[:4000]),view=MemberHistoryView(self.owner_view.bot,member.id)); return
        if self.action=="forgive":
            timeout_cleared=await cog.forgive(interaction.guild.id,member.id)
            msg=f"Forgave {member.mention}: cleared VC detections/score" + (" and removed their Discord timeout." if timeout_cleared else ". I could not confirm the Discord timeout was cleared.")
...
class MemberHistoryView(discord.ui.View):
    ...
    async def all(self,interaction,_):
        if not await _authorized(interaction,self.bot,minimum="admin"): return
        cog=_vc_automod_cog(self.bot); timeout_cleared=await cog.forgive(interaction.guild.id,self.user_id); await interaction.response.send_message("Cleared detections and point infractions" + (" and removed the Discord timeout." if timeout_cleared else ". I could not confirm the Discord timeout was cleared."),ephemeral=True)
...
async def show_automod_page(interaction: discord.Interaction, bot) -> None:
    ...
    await interaction.response.edit_message(embed=discord.Embed(title="Mommy.exe • Auto Mod",description=desc),view=AutomodView(bot))
...
async def show_punishment_ladder_page(interaction: discord.Interaction, bot) -> None:
    ...
    desc += "\n\n**Automatic server bans and kicks are disabled.** Mommy can only warn, remove House Trained, timeout, or remove House Trained + timeout.\n\nActions: `warning`, `remove_role`, `timeout`, `remove_timeout`."
...
async def show_vc_triggers_page(interaction: discord.Interaction, bot) -> None:
    ...
    def _scope_short(t):
        mode=str(t.get("scope_mode") or "everyone")
        n=len(t.get("scope_user_ids") or [])
        if mode=="only_selected": return f"🎯 only {n} selected"
        if mode=="except_selected": return f"🚫 except {n} selected"
        return "🌐 everyone"
    lines=[f"• **{t.get('label','Trigger')}** — {t.get('threshold',3)} hit(s) / {t.get('window_minutes',10)} min → **{t.get('points',1)} pt** • lasts {t.get('history_days',7)}d • {'📣 warning ON' if t.get('spoken_warning',False) else '🔇 warning OFF'} • {_scope_short(t)}{' • built-in' if t.get('builtin') else ''}" for t in cfg.get('triggers') or []]
