...
class VCTriggerModal(discord.ui.Modal, title="Add VC Automod Trigger"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        trigger={"id":uuid.uuid4().hex[:12],"label":str(self.label).strip(),"phrase":str(self.phrase).strip(),"enabled":True,"threshold":threshold,"window_minutes":window,"points":points,"history_days":history_days,"quote_lenient":True,"spoken_warning":False,"warning_style":"random","warning_text":""}
...
async def show_vc_triggers_page(interaction: discord.Interaction, bot) -> None:
    cog=_vc_automod_cog(bot); cfg=await cog.ensure_defaults(interaction.guild) if cog else ((await bot.database.get_guild_profile(interaction.guild.id) or {}).get("vc_automod") or {})
    lines=[f"• **{t.get('label','Trigger')}** — {t.get('threshold',3)} hit(s) / {t.get('window_minutes',10)} min → **{t.get('points',1)} pt** • lasts {t.get('history_days',7)}d • {'📣 warning ON' if t.get('spoken_warning',False) else '🔇 warning OFF'}{' • built-in' if t.get('builtin') else ''}" for t in cfg.get('triggers') or []]
