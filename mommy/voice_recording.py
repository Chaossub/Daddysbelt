...
class VoiceRecordingCog(commands.Cog):
    ...
    async def _handle_segment_ready(self, session: RecordingSession, segment: SpeechSegment) -> None:
        ...
        if caption_channel is not None and self._openai is not None and segment.byte_count >= int(BYTES_PER_SECOND * 0.20):
            try:
    ...
    async def on_voice_state_update(
        ...
        if is_followed and isinstance(after.channel, discord.VoiceChannel):
            session = self.sessions.get(guild.id)
            if session is None:
                await self._start_auto_session(guild, after.channel, member.id)
                return
            if session.auto_started and session.voice_channel_id != after.channel.id:
                vc = guild.voice_client
                if isinstance(vc, voice_recv.VoiceRecvClient) and vc.is_connected():
                    try:
                        ...
                        try:
                            await after.channel.send(f"**Mommy.exe followed {member.display_name} into `{after.channel.name}`.**")
                        except Exception:
                            pass
                    except Exception:
                        log.exception("Could not follow watched member %s to VC %s", member.id, after.channel.id)
                return
    ...
    async def follow_add(self, interaction: discord.Interaction, member: discord.Member) -> None:
        ...
        await interaction.response.send_message(f"I’ll follow **{member.display_name}** into voice chat and start listening automatically.", ephemeral=True)
