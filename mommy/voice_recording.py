...
class VoiceRecordingCog(commands.Cog):
    ...
    async def _handle_segment_ready(self, session: RecordingSession, segment: SpeechSegment) -> None:
        ...
        if text and session.automod_enabled:
            ...
            # 18+ VC Guard receives the same finalized transcript stream but has
            # its own disconnect/appeal policy instead of the punishment ladder.
            child_guard = self.bot.get_cog("ChildVCGuardianCog")
            if child_guard is not None and hasattr(child_guard, "process_transcript"):
                try:
                    await child_guard.process_transcript(
                        guild, segment.user_id, text, voice_channel_id=session.voice_channel_id
                    )
                except Exception:
                    log.exception("18+ child VC guard processing failed in guild %s", guild.id)
