...
class VoiceRecordingCog(commands.Cog):
    ...
    async def _handle_solo_segment(
        ...
        async with self._solo_reply_lock(guild.id):
            ...
            try:
                ...
                if self._is_addressed_to_mommy(user_text):
    ...
    async def _start_auto_session(
        ...
        *,
        ...
        async with self._auto_lock(guild.id):
            ...
            if selected["clips"] and not isinstance(output, discord.TextChannel):
                log.warning("Auto-join clips are enabled in guild %s but no archive/output channel is configured; clip commands will be unavailable.", guild.id)
            ...
            try:
                mode = "automod + recording" if session.record_enabled else "automod/listening"
                await target.send(f"**Mommy.exe joined `{target.name}` for {mode}.**")
    ...
    async def _apply_session_behavior(self, session: RecordingSession, behavior: dict) -> None:
        ...
        session.clips_enabled = clean["clips"]
    ...
    async def _reconcile_auto_join(self, guild: discord.Guild) -> None:
        ...
        if isinstance(vc, voice_recv.VoiceRecvClient) and vc.is_connected():
            try:
                ...
                await self._apply_session_behavior(session, behavior)
                try:
                    await target.send(f"**Mommy.exe moved to `{target.name}` for {reason.replace('-', ' ')}.**")
                except Exception:
                    pass
                return
            except Exception:
    ...
    async def _stop_auto_session(self, guild: discord.Guild, session: RecordingSession) -> None:
        async with self._auto_lock(guild.id):
            ...
            try:
                if session.record_enabled and isinstance(output, discord.TextChannel):
    ...
    async def dashboard_set_active_flag(self, guild: discord.Guild, flag: str, enabled: bool) -> str:
        ...
        label = {"automod":"VC Automod","ai":"AI replies","record":"Full recording/archive","clips":"Rolling clips","captions":"Live captions"}[flag]
    ...
    async def dashboard_clip(self, guild: discord.Guild, user: discord.Member | discord.User) -> str:
        ...
        if not session.clips_enabled:
            return "Rolling clips are disabled for this auto-join target."
        try:
    ...
    async def follow_member_now(self, guild: discord.Guild, member: discord.Member) -> str:
        ...
        if session and session.voice_channel_id == target.id:
            mode = "Automod + Record" if session.record_enabled else "Automod Only"
            return f"Following **{member.display_name}** — I joined **{target.name}** ({mode})."
