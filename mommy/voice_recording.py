...
class VoiceRecordingCog(commands.Cog):
    ...
    def _silence_conversation(self, guild: discord.Guild, user_id: int) -> None:
        self._active_conversations.pop((guild.id, user_id), None)
        self._vc_context.pop((guild.id, user_id), None)
        vc = guild.voice_client
    ...
    async def _handle_solo_segment(
        ...
        if self._is_stop_talking_request(user_text) and (addressed or continuing or mommy_is_speaking):
