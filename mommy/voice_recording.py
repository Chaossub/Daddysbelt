...
SEGMENT_GAP_SECONDS = float(os.getenv("MOMMY_SEGMENT_GAP_SECONDS", "1.35"))
...
def _split_audio_smaller(audio_path: Path, max_bytes: int) -> list[Path]:
    """Retry splitter with extra headroom for Discord multipart overhead."""
    if audio_path.stat().st_size <= max_bytes:
        return [audio_path]
    ffmpeg = _ffmpeg_executable()
    if not ffmpeg or audio_path.suffix.lower() != ".mp3":
    ...
    # Use a much smaller target (~60% of the reported guild limit) so Discord
    # cannot reject the attachment because of multipart overhead or stale limits.
    safe_bytes = max(750_000, int(max_bytes * 0.65))
    seconds = max(30, int((safe_bytes * 8 / 96_000) * 0.82))
    pattern = audio_path.with_name("retry_part_%03d.mp3")
    subprocess.run(
        [
            ffmpeg,
            "-y",
    ...
    return sorted(audio_path.parent.glob("retry_part_*.mp3"))
...
class VoiceRecordingCog(commands.Cog):
    ...
    def __init__(self, bot: MommyExeBot) -> None:
        ...
        self._recording_checkpoint_tasks: dict[int, asyncio.Task] = {}
        self._recording_checkpoint_offsets: dict[int, float] = {}
        ...
        self._last_status_event: dict[int, tuple[str, datetime]] = {}
    ...
    async def cog_unload(self) -> None:
        ...
        for task in self._recording_checkpoint_tasks.values():
            task.cancel()
        self._recording_checkpoint_tasks.clear()
        self._recording_checkpoint_offsets.clear()
    ...
    async def _status_channel(self, guild: discord.Guild) -> discord.TextChannel | None:
        """Use the configured recording/archive channel as Mommy's private VC status log."""
        try:
            config = await self._recording_config(guild)
        except Exception:
            return None
        raw = config.get("status_log_channel_id") or config.get("output_channel_id")
        try:
            channel_id = int(raw)
    ...
    async def status_log(self, guild: discord.Guild, message: str, *, level: str = "info") -> None:
        """Write a concise operational event to the configured staff log channel."""
        self._last_status_event[guild.id] = (message, datetime.now(timezone.utc))
        channel = await self._status_channel(guild)
        if channel is None:
            return
        icon = {"ok": "🟢", "warn": "🟡", "error": "🔴", "info": "🔵"}.get(level, "🔵")
        try:
            await channel.send(f"{icon} **Mommy.exe VC status** • {message}")
        except Exception:
            log.exception("Could not post Mommy.exe VC status in guild %s", guild.id)

    def live_status(self, guild: discord.Guild) -> dict[str, object]:
        """Snapshot used by the admin Status dashboard."""
        ...
        channel = getattr(vc, "channel", None) if connected else None
        last_event = self._last_status_event.get(guild.id)
        return {
            "session": session,
            "connected": connected,
            "listening": listening,
            "voice_channel_id": getattr(channel, "id", None),
            "transcription": self._openai is not None,
            "last_error": self._last_voice_error.get(guild.id),
            "last_event": last_event,
            "hushed": guild.id in self._vc_hushed_guilds,
        }

    def _voice_receive_after(self, guild_id: int, err: Exception | None) -> None:
        if err is None:
            return
        self._last_voice_error[guild_id] = f"{type(err).__name__}: {err}"
        log.error("Voice receive stopped: %r", err)
        ...
        if guild is not None:
            try:
                asyncio.run_coroutine_threadsafe(
                    self.status_log(
                        guild,
                        f"Voice receive stopped unexpectedly: `{type(err).__name__}: {err}`",
                        level="error",
                    ),
                    self.bot.loop,
                )
            except Exception:
                log.exception("Could not schedule voice receive failure status for guild %s", guild_id)
    ...
    async def _solo_silence_timeout(self, session: RecordingSession, user_id: int) -> None:
        try:
            await asyncio.sleep(float(os.getenv("SOLO_AI_SILENCE_SECONDS", "1.35")))
    ...
    def _start_recording_checkpoint(self, session: RecordingSession) -> None:
        old = self._recording_checkpoint_tasks.pop(session.guild_id, None)
        if old is not None:
            old.cancel()
        self._recording_checkpoint_offsets[session.guild_id] = 0.0
        self._recording_checkpoint_tasks[session.guild_id] = asyncio.create_task(
            self._recording_checkpoint_loop(session),
            name=f"mommy-recording-checkpoint-{session.guild_id}",
        )

    async def _recording_checkpoint_loop(self, session: RecordingSession) -> None:
        """Privately checkpoint long recordings so one large finalization is never all-or-nothing."""
        minutes = max(5, int(os.getenv("RECORDING_CHECKPOINT_MINUTES", "15")))
        interval = float(minutes * 60)
        try:
            while self.sessions.get(session.guild_id) is session and not session.closed:
                await asyncio.sleep(interval)
                if self.sessions.get(session.guild_id) is not session or session.closed:
                    return
                if not session.record_enabled or not session.tracks:
                    continue
                guild = self.bot.get_guild(session.guild_id)
                if guild is None:
                    continue
                output = guild.get_channel(session.output_channel_id) if session.output_channel_id else None
                if not isinstance(output, discord.TextChannel):
                    continue
                try:
                    await self._post_recording_checkpoint(guild, output, session, interval)
                except asyncio.CancelledError:
                    raise
                except Exception as exc:
                    self._last_voice_error[guild.id] = f"Checkpoint failed: {type(exc).__name__}: {exc}"
                    log.exception("Long-recording checkpoint failed in guild %s", guild.id)
                    await self.status_log(
                        guild,
                        f"Recording checkpoint failed: `{type(exc).__name__}: {exc}`",
                        level="error",
                    )
        except asyncio.CancelledError:
            pass
        finally:
            current = self._recording_checkpoint_tasks.get(session.guild_id)
            if current is asyncio.current_task():
                self._recording_checkpoint_tasks.pop(session.guild_id, None)

    async def _post_recording_checkpoint(
        self,
        guild: discord.Guild,
        ...
        start_offset = self._recording_checkpoint_offsets.get(guild.id, max(0.0, end_offset - interval_seconds))
        if end_offset <= start_offset + 1.0:
            return

        # Flush any speech sitting in an open segment so this checkpoint includes
        # everything heard up to the checkpoint moment.
        with session.lock:
            for track in session.tracks.values():
                if track.active_segment is not None:
                    session._close_segment(track)
            checkpoint_segments = [
                seg for seg in session.segments
                if start_offset <= seg.start_offset < end_offset
            ]

        seconds = max(1, int(end_offset - start_offset))
        # _mix_recent_clip mixes the latest N seconds; because checkpoints are
        # evenly spaced this is the exact new slice in normal operation.
        wav = await asyncio.to_thread(_mix_recent_clip, session, seconds)
        audio = await asyncio.to_thread(_compress_master, wav)

        rows: list[tuple[float, int, str, str]] = []
        if self._openai is not None and checkpoint_segments:
            sem = asyncio.Semaphore(int(os.getenv("TRANSCRIPTION_CONCURRENCY", "3")))
            results = await asyncio.gather(
                *(self._transcribe_segment(seg, sem) for seg in checkpoint_segments),
                return_exceptions=True,
            )
            for item in results:
                if isinstance(item, Exception):
                    log.warning("A checkpoint speech segment failed transcription", exc_info=item)
                    continue
                seg, transcript = item
                if transcript:
                    rows.append((seg.start_offset, seg.user_id, seg.display_name, transcript))
            rows.sort(key=lambda row: row[0])

        checkpoint_path = session.base_dir / f"checkpoint_{int(end_offset):06d}.txt"
        lines = [
            "Discord VC recording checkpoint",
            f"Server: {guild.name}",
            f"Voice channel ID: {session.voice_channel_id}",
            f"Checkpoint range: {_clock(start_offset)} - {_clock(end_offset)}",
            "",
        ]
        if self._openai is None:
            lines.append("Transcription unavailable.")
        elif not rows:
            lines.append("No transcribable speech was detected in this checkpoint.")
        else:
            for offset, _uid, speaker, transcript in rows:
                lines.append(f"[{_clock(offset)}] {speaker}: {transcript}")
        checkpoint_path.write_text("\n".join(lines), encoding="utf-8")

        max_bytes = max(1_000_000, int(guild.filesize_limit * 0.88))
        parts = await asyncio.to_thread(_split_audio_for_discord, audio, max_bytes)
        await output.send(
            f"💾 **Recording checkpoint** — <#{session.voice_channel_id}> • "
            f"`{_clock(start_offset)} → {_clock(end_offset)}`\n"
            "Session is still recording. This checkpoint is a recovery copy.",
            file=discord.File(checkpoint_path, filename=f"checkpoint_{int(end_offset)}.txt"),
        )
        for index, part in enumerate(parts, start=1):
            label = "Checkpoint audio" if len(parts) == 1 else f"Checkpoint audio {index}/{len(parts)}"
            await output.send(label, file=discord.File(part, filename=f"checkpoint_{int(end_offset)}_{index:02d}{part.suffix}"))

        self._recording_checkpoint_offsets[guild.id] = end_offset
        await self.status_log(
            guild,
            f"Recording checkpoint saved from <#{session.voice_channel_id}> at `{_clock(end_offset)}`. Session is still active.",
            level="ok",
        )
    ...
    async def _segment_watchdog(self, session: RecordingSession) -> None:
        """Close speech chunks after silence and detect a lost/manual VC disconnect."""
        silence_seconds = float(os.getenv("LIVE_CAPTION_SILENCE_SECONDS", "1.35"))
        try:
            while self.sessions.get(session.guild_id) is session and not session.closed:
                await asyncio.sleep(0.20)

                # Discord does not always give us a reliable self voice-state
                # callback when a moderator manually disconnects the bot.
                # Detect the missing voice connection from the live session too.
                guild = self.bot.get_guild(session.guild_id)
                vc = guild.voice_client if guild is not None else None
                disconnected = (
                    guild is not None
                    and session.auto_started
                    and (vc is None or not vc.is_connected() or getattr(vc, "channel", None) is None)
                )
                if disconnected:
                    should_archive = bool(session.record_enabled or session.archive_on_disconnect)
                    if should_archive:
                        session.archive_on_disconnect = True
                    try:
                        await self._stop_auto_session(
                            guild,
                            session,
                            force_finalize=should_archive,
                        )
                    except Exception:
                        log.exception(
                            "Watchdog could not finalize VC session after Mommy lost voice connection in guild %s",
                            session.guild_id,
                        )
                    return
            ...
            with wav_path.open("rb") as audio:
                result = await self._openai.audio.transcriptions.create(
                    ...
                    prompt=(
                        ...
                        "Do not invent words from breaths, hesitation, clipped syllables, background noise, or partial audio. "
                        "Keep incomplete speech incomplete rather than guessing a full sentence. "
                        "For unclear words, use the most plausible English wording only when the audio supports it."
                    ),
                )
    ...
    async def _handle_solo_segment(
        ...
        if segment.user_id not in {member.id for member in humans}:
            return
        if segment.byte_count < int(BYTES_PER_SECOND * 0.55):
            return
        ...
        # During an active conversation, don't treat tiny filler fragments as
        # complete turns. Explicit wake/address phrases are still allowed.
        words = re.findall(r"[A-Za-z0-9']+", user_text)
        if continuing and not addressed and len(words) < 3:
            return

        # Give humans a beat to continue their thought. The old behavior replied
        # almost immediately after every short pause, which made Mommy talk over
        # people and turned STT into lots of tiny fragments.
        await asyncio.sleep(float(os.getenv("MOMMY_REPLY_GRACE_SECONDS", "0.85")))
        if self.sessions.get(guild.id) is not session:
            return
        now = time.perf_counter()
        for track in session.tracks.values():
            if track.user_id == getattr(self.bot.user, "id", None):
                continue
            if track.active_segment is not None:
                return
            if track.last_packet_at is not None and (now - track.last_packet_at) < float(
                os.getenv("MOMMY_HUMAN_SPEECH_GRACE_SECONDS", "0.65")
            ):
                return
    ...
    async def _start_auto_session(
        ...
        *,
        ...
        async with self._auto_lock(guild.id):
            ...
            self._start_recording_checkpoint(session)
            sink = SessionSink(session)
            vc.listen(sink, after=lambda err, gid=guild.id: self._voice_receive_after(gid, err))
            await self._maybe_post_presence_message(guild, session, joined=True)
            mode_bits = []
            if session.record_enabled:
                mode_bits.append("recording")
            if session.automod_enabled:
                mode_bits.append("automod")
            if session.captions_enabled:
                mode_bits.append("captions")
            if session.ai_enabled:
                mode_bits.append("AI replies")
            mode = ", ".join(mode_bits) if mode_bits else "listen-only"
            await self.status_log(
                guild,
                f"Active in {target.mention} • **{mode}**"
                + (f" • following <@{join_target_id}>" if reason == "follow-member" and join_target_id else ""),
                level="ok",
            )
    ...
    async def dashboard_connect_followed(
        self,
        guild: discord.Guild,
        user: discord.Member | discord.User,
    ) -> str:
        """Manually connect Mommy to the best configured followed member who is already in VC."""
        config = await self._recording_config(guild)
        ...
        if not followed:
            return "None of the configured followed members are currently in a voice channel."

        followed.sort(key=lambda row: (row[0], row[1]))
        _, _, channel, behavior, target_id = followed[0]

        session = self.sessions.get(guild.id)
        if session is not None:
            if session.voice_channel_id == channel.id:
                session.join_reason = "follow-member"
                session.join_target_id = target_id
                await self._apply_session_behavior(session, behavior)
                return f"Mommy is already active in {channel.mention} with <@{target_id}>."

            # A manual staff request is allowed to replace an existing auto session.
            if session.auto_started:
                await self._stop_auto_session(
                    guild,
                    session,
                    force_finalize=bool(session.record_enabled or session.archive_on_disconnect),
                )
            else:
                return (
                    "Mommy already has a manually started VC session. "
                    "Use **Stop / Leave** first, then connect to the followed member."
                )
        ...
        await self._start_auto_session(
            ...
            user.id,
            behavior=behavior,
            reason="follow-member",
            join_target_id=target_id,
        )

        new_session = self.sessions.get(guild.id)
        if new_session is None:
            return f"I found <@{target_id}> in {channel.mention}, but Mommy could not connect."

        await self.status_log(
            guild,
            f"Manual staff connect requested by <@{user.id}> • joined {channel.mention} with followed member <@{target_id}>.",
            level="ok",
        )
        modes = []
        if new_session.record_enabled:
            modes.append("recording")
        if new_session.automod_enabled:
            modes.append("automod")
        if new_session.captions_enabled:
            modes.append("captions")
        if new_session.ai_enabled:
            modes.append("AI replies")
        mode_text = ", ".join(modes) if modes else "listen-only"
        return f"Connected Mommy to {channel.mention} with <@{target_id}> • **{mode_text}**."
        ...
        *,
        ...
        async with self._auto_lock(guild.id):
            ...
            if current is not session:
                return
            if not session.auto_started and not force_finalize:
                return
            ...
            if watchdog is not None and watchdog is not asyncio.current_task():
                watchdog.cancel()
            checkpoint = self._recording_checkpoint_tasks.pop(guild.id, None)
            if checkpoint is not None and checkpoint is not asyncio.current_task():
                checkpoint.cancel()
            self._recording_checkpoint_offsets.pop(guild.id, None)
            should_archive = bool(session.record_enabled or session.archive_on_disconnect)
            ...
            try:
                if should_archive and isinstance(output, discord.TextChannel):
                    # _finish_and_post performs transcription before temporary
                    # recording files are removed below.
                    await self._finish_and_post(guild, output, session)
                elif should_archive:
                    log.warning(
                        "Could not post/transcribe automatic recording for guild %s because the output channel is missing.",
                        guild.id,
                    )
            except Exception:
    ...
    async def on_ready(self) -> None:
        first_ready = not getattr(self, "_follow_reconcile_started", False)
        if first_ready:
            self._follow_reconcile_started = True
            asyncio.create_task(self._reconcile_followed_members())
        # on_ready can fire again after a gateway reconnect. That's useful:
        # the staff log will show when Mommy comes back online.
        await asyncio.sleep(1.0)
        for guild in self.bot.guilds:
            await self.status_log(
                guild,
                "Voice system online"
                + (" • transcription ready." if self._openai is not None else " • transcription unavailable (no OpenAI key)."),
                level="ok",
            )
    ...
    async def on_voice_state_update(
        ...
        # connection drop, kicked from VC, etc.), a followed recording must end
        # immediately and be processed exactly like Stop Recording:
        # stop capture -> close segments -> transcribe -> post transcript/audio -> clear session.
        if self.bot.user is not None and member.id == self.bot.user.id:
            if before.channel is not None and after.channel is None:
                ...
                if session is not None:
                    is_follow_recording = (
                        session.join_reason == "follow-member"
                        and bool(session.record_enabled or session.archive_on_disconnect)
                    )
                    is_other_auto_recording = (
                        session.auto_started
                        and bool(session.record_enabled or session.archive_on_disconnect)
                    )
                    if is_follow_recording or is_other_auto_recording:
                        await self.status_log(
                            member.guild,
                            "Voice connection lost/disconnected • finalizing the active recording and transcript.",
                            level="warn",
                        )
                        # Keep the archive intent latched even if follow behavior was
                        # changed immediately before the disconnect event arrived.
                        session.archive_on_disconnect = True
                        try:
                            await self._stop_auto_session(
                                member.guild,
                                session,
                                force_finalize=is_follow_recording,
                            )
                        except Exception:
                            log.exception(
                                "Could not stop/transcribe recording after Mommy was disconnected in guild %s",
                                member.guild.id,
                            )
            return
        ...
        # If the specific followed member that caused this session leaves or
        # moves out of Mommy's current VC, end that recording immediately.
        # Do not roll the same archive forward into another followed target.
        session = self.sessions.get(member.guild.id)
        if (
            session is not None
            and session.auto_started
            and session.join_reason == "follow-member"
            and session.join_target_id == member.id
            ...
            should_archive = bool(session.record_enabled or session.archive_on_disconnect)
            if should_archive:
                # Latch archive intent before any follow-state reconciliation.
                session.archive_on_disconnect = True
            await self.status_log(
                member.guild,
                f"Follow target {member.mention} left/moved VC"
                + (" • finalizing recording/transcript." if should_archive else " • ending session."),
                level="info",
            )
            try:
                await self._stop_auto_session(
                ...
                log.exception(
                    "Could not stop/transcribe followed recording after %s left VC in guild %s",
                    member.id,
                    member.guild.id,
                )
            # After the finished recording/transcript has been posted, check
            # whether another followed member or configured room now warrants
            # a fresh session.
            ...
            try:
                ...
                log.exception(
                    "Auto-join reconciliation failed after followed member left in guild %s",
                    member.guild.id,
                )
            return
    ...
    async def stop_recording(self, interaction: discord.Interaction) -> None:
        ...
        if watchdog is not None:
            watchdog.cancel()
        checkpoint = self._recording_checkpoint_tasks.pop(interaction.guild.id, None)
        if checkpoint is not None and checkpoint is not asyncio.current_task():
            checkpoint.cancel()
        self._recording_checkpoint_offsets.pop(interaction.guild.id, None)
    ...
    async def _finish_and_post(self, guild: discord.Guild, output: discord.TextChannel, session: RecordingSession) -> None:
        ...
        async def _post_audio_parts(send_parts: list[Path], *, retry: bool = False) -> None:
            nonlocal upload_error
            for index, part in enumerate(send_parts, start=1):
                label_base = "Full recording" if len(send_parts) == 1 else f"Recording part {index}/{len(send_parts)}"
                label = f"{label_base} (retry)" if retry else label_base
                try:
        ...
        try:
            await _post_audio_parts(parts)
        except Exception as exc:
            log.warning("Initial full-recording upload failed in guild %s; retrying smaller chunks", guild.id, exc_info=exc)
            await self.status_log(
                guild,
                f"Recording audio upload failed once: `{type(exc).__name__}: {exc}` • retrying smaller chunks.",
                level="warn",
            )
            # If some original parts posted before the failure, leave them there,
            # but retry the full master as smaller recovery chunks so no audio is lost.
            retry_parts: list[Path] = []
            try:
                retry_parts = await asyncio.to_thread(
                    _split_audio_smaller,
                    audio_file,
                    max(1_000_000, int(guild.filesize_limit * 0.90)),
                )
                if retry_parts:
                    upload_error = None
                    await _post_audio_parts(retry_parts, retry=True)
            except Exception as retry_exc:
                upload_error = retry_exc
                log.exception("Retry audio upload failed in guild %s", guild.id)

        if not audio_message_ids or upload_error is not None:
            detail = (
                f"`{type(upload_error).__name__}: {upload_error}`"
                if upload_error is not None
                else "no audio parts were produced"
            )
            await output.send(
                "🔴 **Recording audio upload failed.** The transcript above was saved, "
                f"but Discord did not accept the recording attachment. Reason: {detail}"
            )
            await self.status_log(
                guild,
                f"Recording transcript saved, but audio upload failed: {detail}",
                level="error",
            )
        else:
            await self.status_log(
                guild,
                f"Recording audio posted successfully • `{len(audio_message_ids)}` attachment(s).",
                level="ok",
            )
        ...
        if audio_message_ids and upload_error is None:
            await self.status_log(
                guild,
                f"Recording finalized from <#{session.voice_channel_id}> • `{_clock(session.duration)}` • "
                f"{len(session.tracks)} speaker track(s) • transcript {'ready' if self._openai else 'unavailable'} • audio ready.",
                level="ok",
            )
