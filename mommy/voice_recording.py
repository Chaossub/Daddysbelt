...
class VoiceRecordingCog(commands.Cog):
    ...
    def _elevenlabs_enabled(self) -> bool:
    ...
    async def _synthesize_speech(
        ...
        *,
        ...
        """Create an MP3 using ElevenLabs when configured, otherwise OpenAI TTS."""
        ...
        if api_key and voice_id:
            ...
            # Optional tuning knobs. If they are not set, the saved ElevenLabs
            # voice settings are used unchanged.
            ...
            for env_name, field in (
                ...
                if raw:
                    try:
                        voice_settings[field] = float(raw)
                    except ValueError:
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.post(
                    ...
                    if response.status >= 400:
                        ...
                        raise RuntimeError(f"ElevenLabs TTS failed ({response.status}): {detail}")
                    path.write_bytes(await response.read())
            return "ElevenLabs"

        if self._openai is None:
            raise RuntimeError("No TTS provider is configured. Add ElevenLabs or OpenAI credentials.")
        voice = openai_voice
        ...
        async with self._openai.audio.speech.with_streaming_response.create(
            ...
            instructions=instructions or (
                "Adult feminine voice. Calm, confident, commanding, motherly, dry and mildly sassy. "
    ...
    async def preview_tts_voice(self, guild: discord.Guild, member: discord.Member, voice: str) -> str:
        ...
        using_elevenlabs = self._elevenlabs_enabled()
        if not using_elevenlabs:
            if self._openai is None:
        ...
        label = "elevenlabs" if using_elevenlabs else voice
        ...
        try:
            ...
            if provider == "ElevenLabs":
                return "Previewed your configured **ElevenLabs** voice. Mommy is already using this voice for live TTS."
    ...
    async def _solo_silence_timeout(self, session: RecordingSession, user_id: int) -> None:
        try:
            await asyncio.sleep(float(os.getenv("SOLO_AI_SILENCE_SECONDS", "1.2")))
    ...
    async def _segment_watchdog(self, session: RecordingSession) -> None:
        ...
        silence_seconds = float(os.getenv("LIVE_CAPTION_SILENCE_SECONDS", "1.2"))
        try:
    ...
    async def _generate_solo_reply(
        ...
        *,
        ...
        history_docs = await self.bot._load_memory(guild_id, user_id, limit=10)  # noqa: SLF001
        ...
        if human_count == 1 and joey_id.isdigit() and int(joey_id) == user_id:
            extra = (
                " This speaker is Joey and he is alone with you. You may roast him more aggressively, especially "
                "if he is rude to you. Occasionally, not repeatedly, you may call him a 'small little lonely man'."
            )
        ...
        response = await self._openai.responses.create(
            ...
            instructions=(
                MOMMY_SYSTEM_PROMPT
                + " Keep spoken VC replies concise, usually one or two sentences and rarely over 40 words."
                + " If someone is rude, insulting, dismissive, or shit-talks you, you may sass them back sharply and humorously."
                + extra
            ),
            input=prompt,
    ...
    def _touch_conversation(self, guild_id: int, user_id: int) -> None:
        self._active_conversations[(guild_id, user_id)] = time.monotonic() + float(
            os.getenv("MOMMY_CONVERSATION_WINDOW_SECONDS", "45")
        )
    ...
    async def _speak_solo_reply(self, guild: discord.Guild, session: RecordingSession, text: str) -> str | None:
        ...
        if not isinstance(channel, discord.VoiceChannel) or len(self._human_members(channel)) not in {1, 2}:
            return None
        vc = guild.voice_client
        if not isinstance(vc, voice_recv.VoiceRecvClient) or not vc.is_connected():
            return None
    ...
    async def play_punishment_sound(self, guild: discord.Guild) -> None:
        """Play a short punishment sting in the current VC.

        If MOMMY_PUNISHMENT_SOUND_PATH points at an audio file, use it. Otherwise
        synthesize a short "You're banned." line using Mommy's normal TTS voice.
        """
        vc = guild.voice_client
        if not isinstance(vc, voice_recv.VoiceRecvClient) or not vc.is_connected():
            return
        ...
        if path is None or not path.exists():
            if not self._elevenlabs_enabled() and self._openai is None:
                return
    ...
    async def _handle_solo_segment(
        ...
        async with self._solo_reply_lock(guild.id):
            ...
            try:
                ...
                # One human: normal conversational participation. Two humans: she
                # only joins when addressed, shit-talked, or already in an active
                # conversation with that speaker.
                if len(humans) == 2:
                    addressed = self._is_addressed_to_mommy(user_text)
                    continuing = self._conversation_is_active(guild.id, segment.user_id)
                    if not addressed and not continuing:
                        return
    ...
    async def _start_auto_session(self, guild: discord.Guild, target: discord.VoiceChannel, started_by_id: int) -> None:
        async with self._auto_lock(guild.id):
            ...
            try:
                await target.send(f"**Mommy.exe joined `{target.name}`. Clock in, comedians.**")
    ...
    async def _stop_auto_session(self, guild: discord.Guild, session: RecordingSession) -> None:
        async with self._auto_lock(guild.id):
            ...
            if isinstance(voice_channel, discord.VoiceChannel):
                try:
                    await voice_channel.send(f"**Mommy.exe clocked out of `{voice_channel.name}`.**")
                except (discord.Forbidden, discord.HTTPException, AttributeError):
                    pass
    ...
    async def dashboard_start(self, guild: discord.Guild, user: discord.Member | discord.User, target: discord.VoiceChannel) -> str:
        ...
        # Dashboard/manual sessions should not be auto-stopped by empty-room logic.
    ...
    async def dashboard_clip(self, guild: discord.Guild, user: discord.Member | discord.User) -> str:
        ...
        try:
            ...
            await output.send(
                ...
                file=discord.File(
                    clip_audio,
                    filename=f"mommy_clip_{int(time.time())}.mp3" if clip_audio.suffix.lower() == ".mp3" else "mommy_clip.wav",
                ),
            )
            return f"Saved the previous {_clock(duration)} to {output.mention}."
        except Exception as exc:
    ...
    async def follow_member_now(self, guild: discord.Guild, member: discord.Member) -> str:
        """Immediately follow a configured member if they are already in voice.

        This is also used by startup reconciliation so Follow Members works even
        when Mommy.exe comes online after the member is already sitting in VC.
        """
        ...
        if not output_id:
            return (
                f"I'm following **{member.display_name}**, but I can't start listening yet because "
                "**Voice → Recording Setup** has no archive/output channel configured."
            )
        output = guild.get_channel(int(output_id))
        if not isinstance(output, discord.TextChannel):
            return (
                f"I'm following **{member.display_name}**, but the configured recording output channel is missing. "
                "Run **Voice → Recording Setup** again."
            )
        ...
        if session is None:
            ...
            session = self.sessions.get(guild.id)
            if session is None:
                return f"I tried to join **{target.name}** for {member.display_name}, but couldn't connect. Check my VC permissions and Render logs."
            return f"Following **{member.display_name}** — I joined **{target.name}** and started listening/buffering now."

        if session.voice_channel_id == target.id:
            return f"Following **{member.display_name}** — I'm already listening in **{target.name}**."

        # Do not hijack a manually-started recording. Follow mode owns only automatic sessions.
        if not session.auto_started:
            return (
                f"Following **{member.display_name}**, but I'm currently in a manually-started VC session. "
                f"I'll follow them after that session ends."
            )
        ...
        if isinstance(vc, voice_recv.VoiceRecvClient) and vc.is_connected():
            try:
                ...
                try:
                    await target.send(f"**Mommy.exe followed {member.display_name} into `{target.name}`.**")
                except Exception:
                    pass
                return f"Following **{member.display_name}** — I moved to **{target.name}** and kept listening."
            except Exception as exc:
                log.exception("Could not immediately follow watched member %s to VC %s", member.id, target.id)
                return f"I couldn't move to **{target.name}**: {type(exc).__name__}. Check my VC permissions and Render logs."

        # Voice client disappeared while the session object remained; recover cleanly.
        self.sessions.pop(guild.id, None)
        try:
            session.close_audio_files()
        except Exception:
        ...
        await self._start_auto_session(guild, target, member.id)
        if guild.id in self.sessions:
            return f"Following **{member.display_name}** — I reconnected to **{target.name}** and started listening."
        return f"I'm following **{member.display_name}**, but I couldn't connect to **{target.name}**."

    async def _reconcile_followed_members(self) -> None:
        """Join followed members who were already in VC when Mommy.exe started/restarted."""
        await self.bot.wait_until_ready()
        for guild in self.bot.guilds:
            try:
                config = await self._recording_config(guild)
                follow_ids = [int(v) for v in (config.get("follow_user_ids") or [])]
                for user_id in follow_ids:
                    member = guild.get_member(user_id)
                    if member and member.voice and isinstance(member.voice.channel, discord.VoiceChannel):
                        result = await self.follow_member_now(guild, member)
                        log.info("Follow reconciliation in guild %s for %s: %s", guild.id, user_id, result)
                        # One automatic VC session per guild. Stop after the first followed member found in voice.
                        if guild.id in self.sessions:
                            break
            except Exception:
                log.exception("Could not reconcile followed members in guild %s", guild.id)
    ...
    async def on_voice_state_update(
        ...
        guild = member.guild
        config = await self._recording_config(guild)
        follow_ids = {int(value) for value in (config.get("follow_user_ids") or [])}
        is_followed = member.id in follow_ids

        # Followed members take priority over the fixed auto-record room.
        if is_followed and isinstance(after.channel, discord.VoiceChannel):
            result = await self.follow_member_now(guild, member)
            log.info("Follow event in guild %s for %s: %s", guild.id, member.id, result)
            return

        if is_followed and after.channel is None:
            await asyncio.sleep(2.0)
            session = self.sessions.get(guild.id)
            if session and session.auto_started:
                current = guild.get_channel(session.voice_channel_id)
                watched_still_here = False
                if isinstance(current, discord.VoiceChannel):
                    watched_still_here = any((not m.bot) and m.id in follow_ids for m in current.members)
                if not watched_still_here:
    ...
    async def follow_add(self, interaction: discord.Interaction, member: discord.Member) -> None:
        ...
        if not decision.allowed:
            ...
            return
        config = await self._recording_config(interaction.guild)
        values = [int(v) for v in (config.get("follow_user_ids") or [])]
        if member.id not in values:
            values.append(member.id)
            await self.bot.database.set_voice_recording_follow_users(interaction.guild.id, values)
    ...
    async def follow_remove(self, interaction: discord.Interaction, member: discord.Member) -> None:
        ...
        if not decision.allowed:
            ...
            return
        config = await self._recording_config(interaction.guild)
        values = [int(v) for v in (config.get("follow_user_ids") or []) if int(v) != member.id]
        await self.bot.database.set_voice_recording_follow_users(interaction.guild.id, values)
    ...
    async def start_recording(
        ...
        notice = f"**Mommy.exe joined `{target.name}`. Clock in, comedians.**"
        try:
    ...
    async def clip_recent(self, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            await output.send(
                ...
                file=discord.File(clip_audio, filename=f"mommy_clip_{int(time.time())}.mp3" if clip_audio.suffix.lower() == ".mp3" else "mommy_clip.wav"),
            )
            await interaction.followup.send(f"Saved the last {_clock(duration)} to {output.mention}.", ephemeral=True)
        except Exception as exc:
