...
class HandCardButton(discord.ui.Button):
    def __init__(self, hand_view: "HandView", index: int, card: str):
        ...
        self.card = card
        # Keep the controls compact. The full card text lives in the embed above,
        # so players only need to match the number they want to play.
        label = f"✍️ {index + 1}" if card == BLANK_TOKEN else str(index + 1)
        super().__init__(
            label=label,
...
class RoundView(discord.ui.View):
    ...
    async def play(self, interaction, _):
        ...
        hand_embed = discord.Embed(
            ...
            description="Choose by tapping the matching number below.",
        )
        for i, card in enumerate(player.hand, 1):
            shown = "✍️ **Write-in blank**" if card == BLANK_TOKEN else card
            hand_embed.add_field(name=f"{i}.", value=shown, inline=False)
