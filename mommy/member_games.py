...
class CAHSession:
    ...
    def fill_hand(self, player: PlayerState, *, initial: bool = False, avoid: set[str] | None = None) -> None:
        ...
        if initial:
            ...
            wanted_blanks = max(0, min(int(self.rules.starting_blank_count), self.rules.hand_size))
...
class BlankChanceModal(discord.ui.Modal, title="Write-in settings"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if starting < 0 or starting > hand_size:
            await interaction.response.send_message(
                f"Starting write-ins must be between 0 and your hand size ({hand_size}).",
                ephemeral=True,
            )
            return
