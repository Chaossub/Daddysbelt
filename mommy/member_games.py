...
class CAHSession:
    ...
    def fill_hand(self, player: PlayerState, *, initial: bool = False, avoid: set[str] | None = None) -> None:
        # Keep every unused card exactly where it is. Only empty hand slots are
        # refilled. A write-in blank is now a RANDOM draw result instead of a
        # fixed number that the game forces back into every hand.
        ...
        while len(player.hand) < self.rules.hand_size:
            if blank_chance > 0 and random.random() < blank_chance:
...
class BlankChanceModal(discord.ui.Modal, title="Write-in blank chance"):
    chance = discord.ui.TextInput(
        label="Chance a new card is a write-in (%)",
        placeholder="15",
        required=True,
        max_length=2,
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        raw = str(self.chance.value).strip()
        try:
            value = int(raw)
        except ValueError:
            await interaction.response.send_message("Enter a whole-number percentage.", ephemeral=True)
            return

        if value < 0 or value > 50:
            await interaction.response.send_message(
                "Choose a write-in draw chance between 0% and 50%.",
                ephemeral=True,
            )
            return

        self.setup_view.session.rules.blank_chance_percent = value
...
class GameSetupView(discord.ui.View):
    ...
    def embed(self):
        ...
        embed.add_field(name="Hand", value=f"{r.hand_size} cards • {r.blank_chance_percent}% write-in chance", inline=True)
...
def lobby_embed(session: CAHSession) -> discord.Embed:
    ...
    embed.set_footer(text=f"{mode} • {session.rules.blank_chance_percent}% write-in draw chance • first to {session.rules.score_to_win}")
    return embed
