...
class CAHSession:
    ...
    async def submit(self, user_id: int, cards: list[str], hand_indexes: list[int]) -> tuple[bool, str]:
        async with self.lock:
            ...
            # Played regular white cards leave the hand immediately and are
            # returned to a random position in the shared draw pool. Write-in
            # blanks are virtual slots and are not added to the white-card pool.
            for idx in sorted(set(hand_indexes), reverse=True):
                if 0 <= idx < len(player.hand):
                    used_card = player.hand.pop(idx)
                    self.discard_white(used_card)
            # Refill only the slots that were played. A just-used card is now
            # eligible to return by chance because it was shuffled back into the pool.
