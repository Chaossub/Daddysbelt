...
class CAHSession:
    ...
    async def _submission_timeout(self, round_number: int) -> None:
        try:
            await asyncio.sleep(90)
            if self.status != "playing" or self.round_number != round_number:
                return
            ...
            # If anyone submitted, continue with those submissions. If nobody
            # submitted, just advance to a fresh round instead of deadlocking.
            if self.submissions:
    ...
    async def _czar_timeout(self, round_number: int) -> None:
        try:
            await asyncio.sleep(60)
            if self.status != "judging" or self.round_number != round_number or not self.submissions:
                return
    ...
    async def start_round(self) -> None:
        ...
        self._submission_timeout_task = asyncio.create_task(
            self._submission_timeout(self.round_number)
        )
    ...
    def round_embed(self) -> discord.Embed:
        ...
        embed = discord.Embed(title=f"🃏 Round {self.round_number}", description=f"### {prompt_text}")
        ...
        embed.add_field(name="🏆 Score", value="\n".join(f"{self.display_player(p.user_id)} — **{p.score}**" for p in scores) or "No scores yet.", inline=False)
        embed.set_footer(text=f"90s to play • First to {self.rules.score_to_win} • {self.rules.ai_style.replace('_', ' ').title()}")
        return embed
    ...
    async def begin_judging(self) -> None:
        ...
        self._czar_timeout_task = asyncio.create_task(
            self._czar_timeout(self.round_number)
        )

    def judging_embed(self, ordered: list[Submission]) -> discord.Embed:
        black = self.current_black or {}
        prompt_text = _display_prompt(black.get("text", ""), int(black.get("pick", 1)))
        embed = discord.Embed(title="👑 Pick a Winner", description=f"### {prompt_text}")
        for i, submission in enumerate(ordered):
            embed.add_field(name=f"{chr(65+i)}", value=" / ".join(submission.cards), inline=False)
        embed.set_footer(text="Anonymous until picked • 60s")
        return embed
    ...
    async def finish_round(self, winner_id: int, reason: str = "") -> None:
        ...
        embed = discord.Embed(title="🏆 Round Winner", description=f"**{self.display_player(winner_id)}**")
        if winner_submission:
            embed.add_field(name="Winning Card", value=" / ".join(winner_submission.cards), inline=False)
        if reason:
            embed.add_field(name="Czar's Take", value=_short(reason, 500), inline=False)
        ...
        if self.players[winner_id].score >= self.rules.score_to_win:
            ...
            embed.description = f"**{self.display_player(winner_id)} wins!**"
...
class GameSetupView(discord.ui.View):
    ...
    def embed(self):
        ...
        embed = discord.Embed(title="🃏 New Game", description="Set up the table, then open the lobby.")
        embed.add_field(name="Packs", value=f"{len(self.session.selected_pack_ids)} selected", inline=True)
        embed.add_field(name="Czar", value=czar, inline=True)
        embed.add_field(name="Humor", value=r.ai_style.replace('_',' ').title(), inline=True)
        embed.add_field(name="Hand", value=f"{r.hand_size} cards • {r.blank_per_hand} write-in{'s' if r.blank_per_hand != 1 else ''}", inline=True)
        embed.add_field(name="Win", value=f"First to {r.score_to_win}", inline=True)
        embed.add_field(name="Players", value=f"Mommy {'✅' if r.mommy_player else '❌'} • Daddy {'✅' if r.daddy_player else '❌'}", inline=True)
        return embed
...
def lobby_embed(session: CAHSession) -> discord.Embed:
    embed = discord.Embed(title="🃏 Game Lobby", description="Grab a seat. The host starts when everyone’s ready.")
    ...
    embed.set_footer(text=f"{mode} • First to {session.rules.score_to_win}")
    return embed
...
class RoundView(discord.ui.View):
    ...
    async def play(self, interaction, _):
        ...
        hand_embed = discord.Embed(
            ...
            description="Pick your answer below.",
        )
        hand_embed.set_footer(text=f"Pick {need} • 90s")
        await interaction.response.send_message(
...
class CAHHomeView(discord.ui.View):
    ...
    @discord.ui.button(label="Packs",emoji="🗃️",style=discord.ButtonStyle.primary)
    async def packs(self,interaction,_):
        embed=discord.Embed(title="🗃️ Packs", description="Pick a deck or make your own.")
        await interaction.response.send_message(embed=embed,view=ManagePacksView(self.cog,interaction.user.id),ephemeral=True)

    @discord.ui.button(label="Rules",emoji="❓",style=discord.ButtonStyle.secondary)
    async def help(self,interaction,_):
        embed=discord.Embed(title="❓ Rules", description="Join the lobby, play the funniest answer, and let the Czar pick the winner. Write-ins let you make your own answer.")
        embed.add_field(name="Czar", value="Rotate between players, use AI for small lobbies, or let AI judge every round.", inline=False)
        embed.add_field(name="Packs", value="Use built-in, personal, or server packs — or mix several together.", inline=False)
        await interaction.response.send_message(embed=embed,ephemeral=True)
...
class MemberHubView(discord.ui.View):
    ...
    @discord.ui.button(label="Play Cards",emoji="🃏",style=discord.ButtonStyle.primary,custom_id="mommy:member:cah")
    async def cah(self,interaction,_):
        ...
        embed=discord.Embed(title="🃏 Cards Against Humanity-ish",description="Start a game or mess with your card packs.")
...
def member_hub_embed(guild: discord.Guild) -> discord.Embed:
    embed=discord.Embed(title="🎮 Games",description="Pick something to play.")
    embed.add_field(name="🃏 Cards Against Humanity-ish",value="Mix packs • write-ins • Mommy & Daddy • human or AI Czar",inline=False)
