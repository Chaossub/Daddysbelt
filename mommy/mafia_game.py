...
class MafiaSession:
    ...
    async def start(self) -> None:
        ...
        self.phase = "night"
        await self._dm_roles()
        await self.start_night()

    async def _dm_roles(self) -> None:
        mafia_ids = [p.user_id for p in self.players.values() if p.role == "mafia"]
        for player in self.players.values():
            ...
            if player.role == "mafia" and len(mafia_ids) > 1:
                teammates = [self.display(uid) for uid in mafia_ids if uid != player.user_id]
                embed.add_field(name="Your Mafia team", value=" • ".join(teammates), inline=False)
            try:
                await member.send(embed=embed)
            except discord.HTTPException:
                log.info("Could not DM Mafia role to %s", player.user_id)

    def night_embed(self) -> discord.Embed:
        embed = discord.Embed(
            title=f"🌙 Night {self.night_number}",
            description="The town goes quiet. If your role has a night action, use the button below.",
        )
...
class NightPublicView(discord.ui.View):
    def __init__(self, session: MafiaSession):
        super().__init__(timeout=session.rules.night_seconds + 15)
        self.session = session

    @discord.ui.button(label="Night Action", emoji="🌙", style=discord.ButtonStyle.primary)
...
class NightActionView(discord.ui.View):
    def __init__(self, session: MafiaSession, player: MafiaPlayer):
        super().__init__(timeout=session.rules.night_seconds)
        self.session = session
        self.player = player
...
class DayHostView(discord.ui.View):
    def __init__(self, session: MafiaSession):
        super().__init__(timeout=session.rules.discussion_seconds + 15)
        self.session = session
...
class VotePublicView(discord.ui.View):
    def __init__(self, session: MafiaSession):
        super().__init__(timeout=session.rules.vote_seconds + 15)
        self.session = session
...
class VoteChoiceView(discord.ui.View):
    def __init__(self, session: MafiaSession, voter: MafiaPlayer):
        super().__init__(timeout=session.rules.vote_seconds)
