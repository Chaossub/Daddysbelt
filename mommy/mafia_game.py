...
class MafiaSession:
    ...
    async def _dm_roles(self) -> None:
        ...
        for player in self.players.values():
            ...
            if not member:
                continue
            embed = self.role_card_embed(player)
            try:
                await member.send(
                    content="🎭 **Mafia has started — here is your role.**",
                    embed=embed,
                )
            except discord.HTTPException:
                log.info("Could not DM Mafia role to %s; player can use View My Role in-game.", player.user_id)

    def night_embed(self) -> discord.Embed:
        embed = discord.Embed(
            title=f"🌙 Night {self.night_number}",
            description=(
                "The town goes quiet. If your role has a night action, use **🌙 Night Action**.\n"
                "Forgot your role? **🎭 View My Role** shows it privately."
            ),
        )
