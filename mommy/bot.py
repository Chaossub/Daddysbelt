...
class MommyExeBot(commands.Bot):
    ...
    async def setup_hook(self) -> None:
        ...
        await self.load_extension("mommy.child_vc")
        await self.load_extension("mommy.appeals")
    ...
    async def on_ready(self) -> None:
        ...
        for guild in self.guilds:
            ...
            child_guard = self.get_cog("ChildVCGuardianCog")
            if child_guard is not None and hasattr(child_guard, "ensure_defaults"):
                await child_guard.ensure_defaults(guild)
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        # In DMs, !mommy opens the universal moderation appeal inbox.
        # This runs before normal AI/chat handling so members can appeal even
        # while timed out, disconnected from VC, or missing access roles.
        if message.guild is None and content.casefold() in {"!mommy", "!mommy appeal", "!mommy appeals"}:
            appeals = self.get_cog("AppealsCog")
            if appeals is not None and hasattr(appeals, "send_dm_menu"):
                try:
                    if await appeals.send_dm_menu(message):
                        return
                except Exception:
                    log.exception("Could not open Mommy appeal menu for user %s", message.author.id)
                    await message.channel.send("I couldn't load your appeals right now. Please try again in a moment.")
                    return
            await message.channel.send("The appeal system is still starting. Try `!mommy` again in a moment.")
            return
