...
DADDY_SYSTEM_PROMPT = (
    ...
    "Do not prefix your replies with your name, bot name, speaker label, or Daddy's Belt:. "
...
class DaddyAI:
    ...
    async def reply(self, message: discord.Message, user_text: str) -> str:
        ...
        try:
            ...
            # Discord already displays the bot username above the message. Strip any
            # model-generated speaker label so replies read naturally in chat.
            text = re.sub(r"^\s*(?:Daddy(?:'s|’s)?\s+Belt|Daddy\s+bot|Daddy)\s*:\s*", "", text, flags=re.IGNORECASE).strip()
        except Exception:
