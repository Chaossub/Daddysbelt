...
class DaddysBeltBot(commands.Bot):
    def __init__(self, settings: Settings) -> None:
        ...
        # app.py sets this to the live Mommy bot when Mommy is enabled.
        # It lets !daddy in DMs use Mommy's single universal appeal backend.
        self.mommy_bot = None
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        # !daddy in DMs opens the same universal appeal inbox as !mommy.
        # Daddy proxies to Mommy's AppealsCog so both commands share one
        # appeal database and cannot create separate/duplicate review systems.
        if message.guild is None and (message.content or "").strip().casefold() in {
            "!daddy", "!daddy appeal", "!daddy appeals"
        }:
            mommy = self.mommy_bot
            appeals = mommy.get_cog("AppealsCog") if mommy is not None else None
            if appeals is not None and hasattr(appeals, "send_dm_menu"):
                try:
                    if await appeals.send_dm_menu(message):
                        return
                except Exception:
                    log.exception("Could not open shared appeal menu from !daddy for user %s", message.author.id)
                    await message.channel.send("I couldn't load your appeals right now. Please try again in a moment.")
                    return
            await message.channel.send("The appeal system is still starting. Try `!daddy` again in a moment.")
            return
