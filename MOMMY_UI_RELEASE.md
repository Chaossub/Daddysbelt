...
## Follow Members hotfix
- Adding a followed member who is already in VC now makes Mommy.exe join immediately.
- Fresh join/move voice-state events now use the same follow helper.
- On Render restart, Mommy.exe reconciles followed members already sitting in VC and rejoins automatically.
- The dashboard now explains when Recording Setup/output configuration is preventing an automatic follow session from starting.
