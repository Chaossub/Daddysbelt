...
## Daddy ban GIF controls
- Daddy's moderation panel now links directly to **Ban Messages / GIFs**.
- Ban announcements can include an optional image/GIF URL.
- Multiple ban messages can be configured with different GIFs and randomized using the existing member-event selection modes.
- Manual and external Discord bans continue to use the same ban-event announcement path.
