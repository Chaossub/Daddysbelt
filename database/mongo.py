...
class MongoDatabase:
    ...
    async def ensure_guild_profile(
        ...
        defaults = {
            ...
            "vc_automod": {
                ...
                "retention_days": 7,
                "punishment_minutes": [5, 15, 60, 1440],
                "punishment_ladder": [
        ...
        def _upsert() -> dict[str, Any]:
            ...
            self.guilds.update_one(
                {"guild_id": guild.id, "voice_recording": {"$exists": False}},
                {"$set": {"voice_recording": defaults["voice_recording"]}},
            )
            self.guilds.update_one(
                {"guild_id": guild.id, "voice_recording.caption_routes": {"$exists": False}},
                {"$set": {"voice_recording.caption_routes": {}}},
            )
            self.guilds.update_one(
                {"guild_id": guild.id, "voice_recording.follow_user_ids": {"$exists": False}},
                {"$set": {"voice_recording.follow_user_ids": []}},
            )
            self.guilds.update_one(
                {"guild_id": guild.id, "voice_recording.follow_targets": {"$exists": False}},
                {"$set": {"voice_recording.follow_targets": []}},
            )
            self.guilds.update_one(
                {"guild_id": guild.id, "voice_recording.auto_join_channels": {"$exists": False}},
                {"$set": {"voice_recording.auto_join_channels": []}},
            )
            self.guilds.update_one(
                {"guild_id": guild.id, "voice_recording.default_join_behavior": {"$exists": False}},
                {"$set": {"voice_recording.default_join_behavior": defaults["voice_recording"]["default_join_behavior"]}},
            )
            self.guilds.update_one(
                {"guild_id": guild.id, "voice_recording.tts_voice": {"$exists": False}},
                {"$set": {"voice_recording.tts_voice": None}},
            )

            for key in ("verification", "vc_automod", "text_automod"):
                self.guilds.update_one(
                    {"guild_id": guild.id, key: {"$exists": False}},
    ...
    async def set_voice_recording_config(
        self,
        guild_id: int,
        *,
    ...
    async def set_voice_recording_caption_route(
        self,
        guild_id: int,
        voice_channel_id: int,
        text_channel_id: int | None,
    ) -> dict[str, Any] | None:
        key = f"voice_recording.caption_routes.{int(voice_channel_id)}"
    ...
    async def set_voice_recording_auto_channel(
        self,
        guild_id: int,
        ...
        return await asyncio.to_thread(
            ...
            {
                "$set": {
                    "voice_recording.auto_voice_channel_id": (
                        int(voice_channel_id) if voice_channel_id is not None else None
                    ),
                    "last_updated_at": utc_now(),
                }
            },
    ...
    async def set_voice_recording_auto_join_channels(
        self,
        guild_id: int,
        ...
        for raw in targets:
            ...
            clean.append({
                ...
                "behavior": self._clean_voice_join_behavior(raw.get("behavior")),
            })
        return await asyncio.to_thread(
            self.guilds.find_one_and_update,
            {"guild_id": guild_id},
            {"$set": {"voice_recording.auto_join_channels": clean, "last_updated_at": utc_now()}},
            return_document=ReturnDocument.AFTER,
        )

    async def set_voice_recording_follow_targets(
        self,
        guild_id: int,
        targets: list[dict[str, Any]],
    ) -> dict[str, Any] | None:
        ...
        return await asyncio.to_thread(
            ...
            {"guild_id": guild_id},
            {"$set": {
                "voice_recording.follow_targets": clean,
                "voice_recording.follow_user_ids": [item["user_id"] for item in clean],
                "last_updated_at": utc_now(),
            }},
            return_document=ReturnDocument.AFTER,
        )
    ...
    @staticmethod
    def _clean_voice_join_behavior(value: Any) -> dict[str, bool]:
        raw = value if isinstance(value, dict) else {}
        # Clipping is intentionally global for every active VC session.
        # Follow-member behavior only controls automod / AI / full recording / captions.
        return {
    ...
    async def set_voice_recording_default_join_behavior(
        self,
        guild_id: int,
        behavior: dict[str, Any],
    ) -> dict[str, Any] | None:
        clean = self._clean_voice_join_behavior(behavior)
        return await asyncio.to_thread(
            self.guilds.find_one_and_update,
            {"guild_id": guild_id},
            {"$set": {"voice_recording.default_join_behavior": clean, "last_updated_at": utc_now()}},
            return_document=ReturnDocument.AFTER,
        )

    async def set_voice_recording_tts_voice(
        self,
        guild_id: int,
        voice: str | None,
    ) -> dict[str, Any] | None:
        clean = (voice or "").strip().lower() or None
        return await asyncio.to_thread(
            self.guilds.find_one_and_update,
            {"guild_id": guild_id},
            {
                "$set": {
                    "voice_recording.tts_voice": clean,
                    "last_updated_at": utc_now(),
                }
            },
            return_document=ReturnDocument.AFTER,
    ...
    async def set_voice_recording_follow_users(
        self,
        guild_id: int,
        user_ids: list[int],
    ) -> dict[str, Any] | None:
        clean = list(dict.fromkeys(int(value) for value in user_ids))
        return await asyncio.to_thread(
            self.guilds.find_one_and_update,
            {"guild_id": guild_id},
            {
                "$set": {
                    "voice_recording.follow_user_ids": clean,
                    "last_updated_at": utc_now(),
                }
            },
            return_document=ReturnDocument.AFTER,
    ...
    async def set_config_path(self, guild_id: int, path: str, value: Any) -> dict[str, Any] | None:
        return await asyncio.to_thread(
            self.guilds.find_one_and_update,
            {"guild_id": guild_id},
            {"$set": {path: value, "last_updated_at": utc_now()}},
            return_document=ReturnDocument.AFTER,
        )

    async def push_config_path(self, guild_id: int, path: str, value: Any) -> dict[str, Any] | None:
        return await asyncio.to_thread(
            self.guilds.find_one_and_update,
            {"guild_id": guild_id},
            {"$push": {path: value}, "$set": {"last_updated_at": utc_now()}},
            return_document=ReturnDocument.AFTER,
        )

    async def pull_config_path(self, guild_id: int, path: str, query: Any) -> dict[str, Any] | None:
        return await asyncio.to_thread(
            self.guilds.find_one_and_update,
            {"guild_id": guild_id},
            {"$pull": {path: query}, "$set": {"last_updated_at": utc_now()}},
            return_document=ReturnDocument.AFTER,
        )
