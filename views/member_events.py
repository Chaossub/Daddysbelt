...
def member_events_embed(guild: discord.Guild, profile: dict[str, Any]) -> discord.Embed:
    embed = discord.Embed(title="👥 Member Events", description=f"**Managing:** {discord.utils.escape_markdown(guild.name)}\n\nCreate custom messages for joins/departures and announcements for native Discord kick/ban events. These settings never perform moderation actions.")
...
def event_embed(guild: discord.Guild, profile: dict[str, Any], event_type: str) -> discord.Embed:
    ...
    embed = discord.Embed(title=f"{emoji} {name} Messages" + (" + GIFs" if event_type in {"kick", "ban"} else ""), description=f"{'🟢 Enabled' if cfg.get('enabled') else '🔴 Disabled'}\n**Channel:** {channel.mention if channel else 'Not set'}\n**Selection:** {mode_names.get(cfg.get('selection_mode', 'weighted'), 'Weighted random')}\n**Messages:** {len(cfg.get('messages', []))}")
    ...
    if event_type in {"kick", "ban"}:
        embed.add_field(name="GIFs / Images", value="Each message can include an optional direct image or GIF URL. Add multiple messages to randomize both the text and GIF.", inline=False)
...
class MemberEventsView(AuthorizedView):
    ...
    @discord.ui.button(label="Kick Announcement", emoji="👢", style=discord.ButtonStyle.secondary, row=1)
    async def kick(self,i,b): await show_event(i,self,"kick")
    @discord.ui.button(label="Ban Announcement + GIFs", emoji="🔨", style=discord.ButtonStyle.secondary, row=1)
