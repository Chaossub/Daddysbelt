...
def moderation_embed(
    ...
    embed.add_field(
        name="Ban Announcements + GIFs",
        value=(
            "Daddy can post a configured announcement when a moderator/admin bans someone "
            "using Discord's native moderation tools. The bot itself cannot ban or kick members. "
            "Use **Ban Announcements / GIFs** below to manage the message and destination channel."
        ),
        inline=False,
    )
    embed.add_field(
        name="Safety",
        value=(
            "Daddy's Belt cannot kick or ban members. Those actions must be done manually by "
            "a moderator/admin using Discord's native controls. Timeout, duration changes, "
            "timeout removal, and purge require confirmation."
        ),
        inline=False,
    )
    ...
    async def confirm(
        ...
        try:
            if self.action in {"kick", "ban"}:
                await interaction.response.send_message(
                    "Bot-executed kicks and bans are disabled. An administrator must use Discord's native moderation controls.",
                    ephemeral=True,
                )
                return
            else:
...
class ModerationView(AuthorizedView):
    ...
    @discord.ui.button(
        label="Ban Announcements / GIFs",
        emoji="🎞️",
        style=discord.ButtonStyle.secondary,
        row=3,
    )
    async def ban_messages_gifs(
        self,
        interaction: discord.Interaction,
        _: discord.ui.Button,
    ) -> None:
        from views.member_events import EventConfigView, event_embed
        ...
        await interaction.response.edit_message(
            embed=event_embed(interaction.guild, profile, "ban"),
            view=EventConfigView(
                bot=self.bot,
                guild_id=self.guild_id,
                requester_id=self.requester_id,
                database_connected=self.database_connected,
