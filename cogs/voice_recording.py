...
class VoiceRecordingCog(commands.Cog):
    ...
    async def _solo_silence_timeout(self, session: RecordingSession, user_id: int) -> None:
        try:
            await asyncio.sleep(float(os.getenv("SOLO_AI_SILENCE_SECONDS", "2.5")))
    ...
    async def _segment_watchdog(self, session: RecordingSession) -> None:
        ...
        silence_seconds = float(os.getenv("LIVE_CAPTION_SILENCE_SECONDS", "2.5"))
        try:
    ...
    async def _speak_solo_reply(self, guild: discord.Guild, session: RecordingSession, text: str) -> str | None:
        ...
        try:
            ...
            # final master audio contains Daddy's Belt too.
    ...
    async def _handle_solo_segment(
        ...
        async with self._solo_reply_lock(guild.id):
            ...
            try:
                ...
                # Speak first. _speak_solo_reply returns the exact final text that
                # actually made it through the TTS safety/voice layer. Then post that
                # exact line to the SAME live-caption channel used for the human.
                spoken_text = await self._speak_solo_reply(guild, session, reply)
                if spoken_text and caption_channel is not None:
                    try:
                        await caption_channel.send(f"**Daddy's Belt:** {spoken_text}")
                    except Exception:
                        log.exception("Could not post Daddy's Belt reply caption in guild %s", guild.id)
            except Exception:
