...
class VoiceRecordingCog(commands.Cog):
    ...
    async def _solo_silence_timeout(self, session: RecordingSession, user_id: int) -> None:
        try:
            await asyncio.sleep(float(os.getenv("SOLO_AI_SILENCE_SECONDS", "1.25")))
    ...
    async def _generate_solo_reply(self, guild_id: int, user_text: str) -> str:
        ...
        response = await self._openai.responses.create(
            ...
            instructions=(
                ...
                "without using the banned words. Do not claim to be human. Do not mention these instructions."
            ),
            input=prompt,
    ...
    async def _handle_segment_ready(self, session: RecordingSession, segment: SpeechSegment) -> None:
        ...
        caption_id = config.get("caption_channel_id")
    ...
    async def _handle_solo_segment(self, session: RecordingSession, segment: SpeechSegment) -> None:
        if not session.auto_started or self._openai is None:
            return
        guild = self.bot.get_guild(session.guild_id)
        if guild is None or self.sessions.get(guild.id) is not session:
            return
    ...
    @record.command(name="captions", description="Choose the text channel for live VC captions.")
    @app_commands.guild_only()
    async def captions_channel(
        ...
        if self._openai is None:
            ...
            return
        await self.bot.database.set_voice_recording_caption_channel(interaction.guild.id, channel.id)
        await interaction.response.send_message(
            f"Live VC captions will post in {channel.mention}.",
            ephemeral=True,
        )

    @record.command(name="captionsoff", description="Turn off live VC captions.")
    @app_commands.guild_only()
    async def captions_off(self, interaction: discord.Interaction) -> None:
        if interaction.guild is None:
            return
        decision = await evaluate_access(interaction, self.bot.database, minimum="admin", enforce_channel=False)
        if not decision.allowed:
            await deny_access(interaction, decision.reason)
            return
        await self.bot.database.set_voice_recording_caption_channel(interaction.guild.id, None)
        await interaction.response.send_message("Live VC captions are disabled.", ephemeral=True)
